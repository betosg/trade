...
class phpMultiLang
{
   ...
   *     @access public
   *
   *     @param string $objLang_root    language root directory (CWD by default)
   *     @param string $cache_path   directory for cache files. Required, if the caching will be used. CWD by default.
   *     @return void
   */
    function phpMultiLang( $objLang_root=null, $cache_path=null )
    {
        $this->_flag_magic_quotes_runtime = ini_get("magic_quotes_runtime");
        $cwd = getcwd();
        if(!is_null($objLang_root)){
            if(is_dir($path = realpath($objLang_root)) && is_readable($path)){
                $this->_LangRoot = $path;
            } else {
                $this->Error("Invalid language root. The directory '<i>".$objLang_root."</i>' not exists, or not readable. Language root switched to CWD.");
                $this->_LangRoot = $cwd;
            }
        } else {
            $this->_LangRoot = $cwd;
    ...
    *
    *   @param  string $objLang_idx   index for language (EN, RU, FR, ..., whatever you'd like)
    *   @param  string $objLang_path  Language directory(relative to current 'lang_root') By default - '_LangRoot' will be used.
    ...
    */
    function AssignLanguage( $objLang_idx=null, $objLang_path=null, $locale=null )
    {
        if(is_string($objLang_idx) && $objLang_idx){
            if (!isset($this->_LangIdx[$objLang_idx])) {
                $this->_LangIdx[$objLang_idx]['dir'] = $this->_LangRoot;
                $this->_LangIdx[$objLang_idx]['parse']=$this->_LangIdx[$objLang_idx]['cache']=array();
                if(!is_null($objLang_path)) $this->SetLanguageDir($objLang_idx,$objLang_path);
                if(is_array($locale)){
                    if(isset($locale[0]) && isset($locale[1])){
                        $this->_LangIdx[$objLang_idx]['locale']['LC_MODE']=$locale[0];
                        $this->_LangIdx[$objLang_idx]['locale']['LC_OPT'] =$locale[1];
                        return true;
                    }
                    $this->Error("Invalid locale options for language '<i>".$objLang_idx."</i>'.");
                    return false;
                }
                return true;
            }
            $this->Error("Attempt of double-assign language. Language '<i>".$objLang_idx."</i>' was already assigned.");
            return false;
        }
    ...
    *   @param string $objLang_dir    Language directory
    *   @return bool   TRUE on success, FALSE-on errors
    */
    function SetLanguageDir( $objLang_idx=null, $objLang_dir=null )
    {
        if(isset($this->_LangIdx[$objLang_idx])){
            if($dir = realpath($this->_LangRoot."/".$objLang_dir)){
                  $this->_LangIdx[$objLang_idx]['dir'] = $dir;
                  return true;
            }
            $this->Error("Invalid language '<i>".$objLang_idx."</i>' directory. The directory  '<i>".$this->_LangRoot."/".$objLang_dir."</i>'.");
            return false;
        }
        $this->Error("Invalid language index. Language '<i>".$objLang_idx."</i>' not yet assigned.");
        return false;
    }
    ...
    *
    *   @access public
    *   @param  string  $objLang_idx  index of language
    ...
    */
    function AssignLanguageSource( $objLang_idx="", $source=array(), $cache_expire=0 )
    {
        if(isset($this->_LangIdx[$objLang_idx])){
            if(is_string($source)){
                ...
                if(is_file($this->_LangIdx[$objLang_idx]['dir']."/".$source) && is_readable($this->_LangIdx[$objLang_idx]['dir']."/".$source)){
                    if($cache_expire > 0){
                        $this->_LangIdx[$objLang_idx]['cache'][$this->_LangIdx[$objLang_idx]['dir']."/".$source]=$cache_expire;
                        return true;
                    }
                    $this->_LangIdx[$objLang_idx]['parse'][]=$this->_LangIdx[$objLang_idx]['dir']."/".$source;
                    return true;
                }
                //$this->Error("Invalid language source filename. The file '<i>".$this->_LangIdx[$objLang_idx]['dir']."/".$source."</i>' not exists, or not readable.");
                return false;
            }
            if(is_array($source)){
                if(!$source) return true;
                if($this->_LangCurrent){
                    if($this->_LangCurrent == $objLang_idx){
                        $this->__LANGDATA=array_merge($this->__LANGDATA,$source);
                    }
                    return true;
                }
                $this->_LangIdx[$objLang_idx]['parse'][]=$source;
                return true;
            }
            $this->Error("Invalid language '<i>".$objLang_idx."</i>' source type. The source must be an array, or valid filename relative to '<i>".$this->_LangIdx[$objLang_idx]['dir']."</i>'.");
            return false;
        }
        $this->Error("Invalid language index. Language '<i>".$objLang_idx."</i>' not yet assigned.");
        return false;
    ...
    *   @access private
    *   @param string $objLang_idx The index of language.
    *   @return bool TRUE on success, or FALSE on errors.
    */
    function SetLocale($objLang_idx="")
    {

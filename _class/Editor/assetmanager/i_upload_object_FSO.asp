...
Class Upload
  ...
  Public Sub Recieve()
	...
	' AQUI ----------------------------------------------------------------------------------
	' Testando os limites permitidos para trabalkho com arquivos byte byte no servidor
	' se o arquivo é muito grande (acima do que o servidor estiver configurado rpa aceitar) o 
	' Request.BinaryRead gera um erro antes de conseguir retornar o tamanho do arquivo.
	' Desta forma fazendo sse teste antes com On ERRO podemos parar o processamenteo antes
	' ----------------------------------------------------------------------------------------
	...
	If Err.Number <> 0 then
        nErrNum = 2
		sErrMsg = "<center>"
		sErrMsg = sErrMsg & "O arquivo excedeu o tamanho permitido para esse tipo de upload neste servidor.<br><br>"
		sErrMsg = sErrMsg & "This may occur if the web server is configured to limit the size of the file that <br>"
		sErrMsg = sErrMsg & "can be uploaded to the server through ASP script (the Request object).<br>"
		sErrMsg = sErrMsg & "Verify Server's Limits for this kind of uploading.<br><br>"
		sErrMsg = sErrMsg & Err.Description
		sErrMsg = sErrMsg & "</center>"

		Error.Clear
    	response.Write sErrMsg
	    response.end
	End If
	on error goto 0 'dizemq ue deslica o 'resume next'
	' ----------------------------------------------------------------------------------------

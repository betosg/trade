...
<%
' AQUI -------------------------------------------------------------
' Re-escrevo os caminhos relativos ao cliente/banco que esta logado
 Function FindServerByPath()
  Dim auxmappath
  auxmappath = lcase(server.mappath("/"))

  If instr(auxmappath,"wwwroot")>0 then 'LOCAL - conforme o nosso servidor
    if instr(auxmappath,"domains")>0 then
	  FindServerByPath = "SOUTHTECH"
	else
	  FindServerByPath = "LOCAL"
	end if
  else
    if instr(auxmappath,"home")>0 then
	  FindServerByPath = "LOCAWEB v1"
	else 
	  if instr(auxmappath,"httpdocs")>0 then 'LOCAWEB v2
	    FindServerByPath = "LOCAWEB v2"
	  else
	    FindServerByPath = "PLUGIN"
	  end if
	end if
  End If
 End Function
 '- Somente pastas específicas: imgdin, upload, swf, etc... --
 dim auxSP, CLI_NAME
 auxSP = FindServerByPath()
 CLI_NAME = Replace(ucase(Request.Cookies("ATHCSM")("DBNAME")), "ATHCSM_", "")

 '- Somente pastas específicas: imgdin, upload, swf, etc... --
 IF auxSP="LOCAWEB v2"	THEN arrBaseFolder(0) = "/" & CLI_NAME &"/imgdin/"			END IF
 IF auxSP="LOCAL"		THEN arrBaseFolder(0) = "/athcsm4/" & CLI_NAME & "/imgdin/"	END IF 
 arrBaseName(0)   = CLI_NAME & "/imgdin" 
 '------------------------------------------------------------

 '- Todos as pastas do cliente -------------------------------
 'arrBaseFolder(0) = "/athcsm4/" & CLI_NAME & "/" 
 'arrBaseName(0)   = CLI_NAME
 '------------------------------------------------------------

...
if($boolOpcao == "web"){
    ...
    //echo $strResultado = @file_get_contents("https://republicavirtual.com.br/web_cep.php?cep=" . urlencode($strCep) . "&formato=query_string");
    $strResultado = @file_get_contents("https://viacep.com.br/ws/" . urlencode($strCep) . "/json/");
    
    // var_dump($strResultado);
   // print(str_replace('"','',$strResultado));
    //$strResultadoBusca = json_encode($strResultado, true);
    //$strResultadoBusca = json_decode($strResultado);
    $strResultadoBusca =json_decode($strResultado, true);
    
    //echo($strResultadoBusca ->"uf");
    //parse_str($strResultado,$strResultadoBusca);
   // print_r($strResultadoBusca);
   // print_r("<br>".$strResultadoBusca["cep"]);
   // die();
	//switch($strResultadoBusca["resultado"]){
  	//	case "2":	$strTexto = "2<br>Cidade com logradouro único" .
	//							 "<br>".$strResultadoBusca['localidade'].
	...
	$strTexto = "1<br>".(utf8_decode($strResultadoBusca['tipo_logradouro'] )).
			 "<br>".    (utf8_decode(trim($strResultadoBusca['logradouro']) )).
			 "<br>".    (utf8_decode(trim($strResultadoBusca['bairro']) )).
			 "<br>".    (utf8_decode(trim($strResultadoBusca['localidade']) )).
			 "<br>".    (utf8_decode(trim($strResultadoBusca['uf']) ));
	echo $strTexto;
} else {

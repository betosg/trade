...
$intValorPed  = request("var_valor"); //valor do pedido

$strSituacaoPed = "aberto"; // situacao do pedido
$strTipoProd = "card"; // situacao do pedido
...
if(($strAcaoRadio == "") && ($intQtdeImpr > 0)){
	...
	mensagem("err_sql_titulo","err_sql_desc_card",$strErro,"../modulo_CadPF/STGeraCard.php?var_chavereg=".$intCodCredencial,"aviso",1);
	die();
}
else{
	if( (($intQtdeImpr == 0)||($intQtdeImpr == ""))  ||  (($strAcaoRadio == "cobr_none") && ($intQtdeImpr > 0))  ){
		...
		if(is_null($intQtdeImpr)||($intQtdeImpr == "")){
			try{
				$strSQL = "UPDATE sd_credencial SET qtde_impresso = 1 WHERE qtde_impresso IS NULL";
				$objResult = $objConn->query($strSQL);		
			}
			catch(PDOException $e){
				mensagem("err_sql_titulo","err_sql_desc","","../modulo_CadPF/STGeraCard.php?var_chavereg=".$intCodCredencial,"aviso",1);
				die();
			}
			redirect("STCard.php?var_chavereg=".$intCodPF);
		}
		//incremento no número de impressões caso impr = 0
		else if(($intQtdeImpr == 0)||(($strAcaoRadio == "cobr_none") && ($intQtdeImpr >0))){
			try{
				$strSQL = "UPDATE sd_credencial SET qtde_impresso = qtde_impresso + 1 WHERE cod_credencial = ".$intCodCredencial;
				$objResult = $objConn->query($strSQL);
			}
			catch(PDOException $e){
				mensagem("err_sql_titulo","err_sql_desc","","../modulo_CadPF/STGeraCard.php?var_chavereg=".$intCodCredencial,"aviso",1);
		...
		try{
			...
			$objStatement = $objConn->prepare("SELECT sp_gera_ped_impr_card(:in_cod_credencial, :in_valor_ped, :in_situacao, :in_tipo_produto);");
			$objStatement->bindParam(":in_cod_credencial",$intCodCredencial);
			$objStatement->bindParam(":in_valor_ped",$intValorPed);
			$objStatement->bindParam(":in_situacao",$strSituacaoPed);
			$objStatement->bindParam(":in_tipo_produto",$strTipoProd);

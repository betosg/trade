...
for($z=0; $z < count($fp); $z++){
	...
	if((!ctype_digit($first_Number)) and (trim ($fp[$z]) <> "")){
		die(mensagem("err_sql_titulo","err_arq_desc","Arquivo Inválido para Validação.","../modulo_ArquivosValidacao/STArqUploadSICOBass.php","erro",1));
	}else{
		verificaLinha($linha);
	}
}
...
function arqImportado(){
	echo "<script language='javascript' type='text/javascript'>
			alert('O arquivo já foi importado para nossa Base de dados');
		  </script>";
}
function arqErro(){
	echo "<script language='javascript' type='text/javascript'>
			alert('O arquivo está com erro');
		  </script>";
}
						...
						<tr>
						   <td valign="top" align="center"> 
							  <table width="500"  border="0" cellpadding="2" cellspacing="0">
								<tr><td><table><?php include "STHeaderArquivoSICOBass.php"; ?></table></td></tr>
								<tr><td><table><?php include "STHeaderLoteSICOBass.php"; ?></table></td></tr>
								<tr><td><table><?php include "STSegmentoTSICOBass.php"; ?></table></td></tr>
								<tr><td><table><?php include "STSegmentoUSICOBass.php"; ?></table></td></tr>
								<tr><td><table><?php include "STTrailerLoteSICOBass.php"; ?></table></td></tr>
								<tr><td><table><?php include "STTrailerArquivoSICOBass.php"; ?></table></td></tr>
...
<!-- *** VERIFICAÇÃO SE SOMENTE FOI VALIDADO OU IMPORTADO *** -->
								...
								<tr>
									...
									<form name="formArqValida" action="../modulo_ArquivosValidacao/STdialogSICOBass.php" method="post">
									<?php if($_SESSION['ValidaErro'] == ""){?>
										<input type="hidden" name="uploadArquivo" value="<?php echo $_SESSION['novoNome'];?>">		
										<button onClick="confirmar(); return false;">
											<?php echo(getTText("pre_importacao",C_UCWORDS)); ?>
										</button>
										&nbsp;

	...
	function analisaArquivo($prObjConn, $arquivo, $prCodRelat){
		...
		for($z=0; $z < count($fp); $z++){
			...
			if((!ctype_digit($first_Number)) and ($fp[$z] <> "")){
				mensagem("err_sql_titulo","err_arq_desc","Arquivo Inválido para Importação.","STArqUploadCobrCNAB400.php","erro",1);
				die();
			} else{
				verificaLinha($row);
			}
			...
			if ($first_digit == "1") {
				...
				if(($intTipoMovimento == "06") || ($intTipoMovimento == "09")  || ($intTipoMovimento == "15") || ($intTipoMovimento == "16") || ($intTipoMovimento == "17")){
		...
		if ($strMsgErro == ""){
			...
			if(($intCodMov == "06") || ($intCodMov == "09") || ($intCodMov == "15") || ($intCodMov == "16") || ($intCodMov == "17")){
				...
				if ($intIDArquivo != "") {
					...
					$intTipoCarteira 	= isNumber($row, 108, 108,"Tipo Carteira");
					$intCodOcorrencia 	= $intCodMov;
					$intMotOcorrencia 	= preg_replace("/[^0-9]/", "", isAlfa($row, 319, 328));
					...
					$strNossoNumero		= isAlfa($row,136,145);
					//Não lemos no NOSSO NUMERO o que tem antes da 127 a 135 porque são zeros
					//Contudo, se for diferente de zeros acrescenta antes do NOSSO NUMERO para não perder informação
					$strPrefixo			= isAlfa($row,127,135);
					if ($strPrefixo != "000000000") $strNossoNumero = $strPrefixo.$strNossoNumero;
					
					$strNumDocumento	= trim(isAlfa($row,117,126));
					...
					$dblTarifCustas 	= "0000000000000.00"; //substr($dblTarifCustas,-13,11).".".substr($dblTarifCustas,-2);
			...
			mensagem("err_sql_titulo","err_arq_desc",$strMsgErro."Por favor faça a validação do arquivo para analisar os campos que estão com erros.","STArqUploadCobrCNAB400.php","erro",1);
		}

...
function initModuloParams($prModulo) {
	...
	try{
		$strSQL = "SELECT cod_app, dir_app, tabela_app, descritor_grp, grid_default, grid_query, num_per_page, rotulo_app, 
						  menucombo_rotulo, menucombo_valores, menucombo_tipo, menucombo_images
				     FROM sys_app 
	...
	if($objResult->rowCount() > 0) {
		foreach($objResult as $objRS){
			...
			//setsession($strSessionPfx . "_select_orig"      ,replaceParametersSession(str_replace(";","",getValue($objRS,"grid_query"))));
	...
	// Testa se a aplicação tem algum direito,
	// mas não testa UM DIREITO em específico
...
function initDialogParams($prConnLocal, $prOperacao, $prExec, $prCodDado){
	...
	//************ Inicializa os paramêtros da dialog
	switch(strtoupper($prOperacao)){
		case "INS": 
			...
			$retValue["dlg_aviso2"] = "";
			$retValue["dlg_location_aplicar"] = "../" . getsession($strSesPfx . "_dir_modulo") . "/insupddelmastereditor.php?var_oper=INS&var_exec=" . $prExec;
			break;
		case "UPD":
			...
			$retValue["dlg_aviso1"] = getTText("rotulo_dialog",C_NONE);
			$retValue["dlg_aviso2"] = "";
			$retValue["dlg_location_aplicar"] = "../" . getsession($strSesPfx . "_dir_modulo") . "/insupddelmastereditor.php?var_oper=UPD&var_chavereg=" . $prCodDado . "&var_exec=" . $prExec;
			break;
		case "VIE":
			$retValue["dlg_titulo"] = "visualizacao";
			$retValue["dlg_action"] = ($prExec != "") ? $prExec : "insupddelmastereditor.php?var_oper=UPD&var_chavereg=" . $prCodDado;
			$retValue["dlg_aviso1"] = "";
			$retValue["dlg_aviso2"] = "";
			$retValue["dlg_location_aplicar"] = "../" . getsession($strSesPfx . "_dir_modulo") . "/insupddelmastereditor.php?var_oper=UPD&var_chavereg=" . $prCodDado . "&var_exec=" . $prExec;
			break;
	...
	// Está aqui porque é igual para todos
	$retValue["dlg_location_default"] = "../" . getsession($strSesPfx . "_dir_modulo") . "/" . getsession($strSesPfx . "_grid_default");
...
function showButtonsArea($prDialogGrp, $prOperacao, $prCodDialogGrid, $prNumFilhos, $prAviso){
	...
	if($prNumFilhos == 1 || ($prDialogGrp == "000" && $prOperacao != "INS")){
		if($prOperacao == "INS" || $prOperacao == "UPD"){
			$strAux = "<button onClick=\"ok('" . $prOperacao . "'); return false;\">" . getTText("ok",C_UCWORDS) . "</button>
					   <button onClick=\"cancelar(); return false;\">" . getTText("cancelar",C_UCWORDS) . "</button>
		...
		if($prOperacao == "INS"){
			$strAux = "<button onClick=\"cancelar(); return false;\">" . getTText("cancelar",C_UCWORDS) . "</button>
					   <button onClick=\"aplicar('UPD'); return false;\">" . getTText("aplicar",C_UCWORDS) . "</button>";
		}
		elseif($prOperacao == "UPD"){
			$strAux = "<button onClick=\"submeterFormRel('" . $prDialogGrp . "','" . $prCodDialogGrid . "'); return false;\">" . getTText("aplicar",C_UCWORDS) . "</button>";
		}
	}
	...
	if($strAux != ""){
		$retValue = "
				<tr align=\"left\">
				  <td height=\"10\" colspan=\"2\" class=\"destaque_med\" style=\"padding-top:5px; padding-right:25px\">" . getTText("campos_obrig",C_NONE) . "</td>
				</tr>
...
function VerificaModoExportacao($prAcao, $prFileName){
	//Define uma variável booleana afim de verificar se é um tipo de exportação ou não
	$boolIsExportation = ($prAcao == ".xls") || ($prAcao == ".doc") || ($prAcao == ".pdf");
	//Exportação para excel, word e adobe reader
	if($boolIsExportation){
		if($prAcao == ".pdf"){
			//Redireciona para página que faz a exportação para adode reader

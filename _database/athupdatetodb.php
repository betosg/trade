	...
	if(strpos($strCampo,FIELD_PREFIX) === 0){
		...
		switch(strtolower($strAuxType)){
			...
			case "moeda4cd": if($strAuxValue == "") {
							   ...
							   //$strAuxValue = number_format((double) $strAuxValue,4);
							   $strAuxValue = str_replace(".","",$strAuxValue);
							   $strAuxValue = str_replace(",",".",$strAuxValue);
							 }
							 break;
			...
			case "captcha":  
							 ...
							 if (!$objCaptcha->check($strAuxValue) && $strAuxValue != "") {
								//mensagem("err_dados_titulo","err_dados_captcha","","javascript:history.back()","erro",1);
								messageDlg(C_MSG_ERRO,"Mensagem / Message",getTText("Captcha incorreto",C_NONE),"","javascript:history.back()",1);
								die();
							 }
							 break;
		}
...
if(!$boolMyFRequired) {
	//Este é um teste antigo, não deve ser usado, os testes de campos devem ser em javascript
	...
	if($strMyTbSetFields != ""){
		...
		try{
			...
			messageDlg(C_MSG_ERRO,"Mensagem / Message","",$e->getMessage() . "<br>SQL: ".$strSqlUpdateToDB,"",1);
...
if(EXIBE_MENSAGEM != 0){
	...
	messageDlg(C_MSG_INFO,"Mensagem / Message",getTText("Cadastro alterado",C_NONE),"",DEFAULT_LOCATION,1);
}
else{
	if(DEFAULT_LOCATION != "") {
		...
		messageDlg(C_MSG_INFO,"Mensagem / Message","",getTText("Operacao realizada",C_NONE),"",1);
	}
} 
?>
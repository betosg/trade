		...
		<?php
			$strSQL = " SELECT rotulo, executor, categoria, target FROM sys_bookmark WHERE cod_app = " . getsession($strSesPfx . "_chave_app") . " ORDER BY ordem " ;
			$objResult = $objConn->query($strSQL);
			
			if($objResult->rowCount() > 0){
				
				//athBeginShapeBox("205","",getTText("bookmark",C_NONE),CL_CORBAR_SHAPE);
				athBeginShapeBox("205","","",CL_CORBAR_SHAPE);
				...
				foreach($objResult as $objRS){
				    $strExecutor = getValue($objRS,"executor");
					...
					if(getValue($objRS,"categoria") != $strCategoria){
						...
						echo("
									...
									<tr onClick=\"collapseItem(" . $intI . ");\" class='bookmark_group'>
										<td width='99%'>" .  getTText(getValue($objRS,"categoria"),C_NONE) . "</td>
					...
					echo("						<tr>
													<td class='bookmark_item'>
														<a href='" . $strExecutor .  "' target='" . $strTarget . "'>" . getTText(getValue($objRS,"rotulo"),C_NONE) . "</a>
													</td>
												</tr>
						");

...
$strReturnOk  = request("var_return");
...
//print_r($arrCfgDialog );

if (getsession("var_return") != ""){
	$arrCfgDialog["dlg_location_default"] = getsession("var_return");
}
...
//echo "key".$arrCfgDialog["dlg_campo_chave"];
...
<html>
	<head>
		...
		function ok(prOperacao){
			if (validateRequestedFields("formeditor_000")==true) {
				//alert("<?php// echo($arrCfgDialog["dlg_location_default"]); ?>")
				if(prOperacao == "INS" || prOperacao == "UPD" || prOperacao == "DEL") {
					...
					//alert(strLocation);
					//}else{
					//	var strLocation = (window.opener == null || window.opener == "undefined") ? prReturn : "";
					
					
					//INI: POPUP?!
					// A DIALOG tenta reconhecer que foi aberta em POPUP, por existir um 'opener'
					// ou não, e neste caso após a ação a dialog(window) deve ser fechada. Quem deve fechá-la é a 
					// próxima página que esta sendo chamada, no caso um das [...]ToDB, desta forma o DEFAULT_LOCATION 
					// é modificado aqui, para uma página DUMMY que fica sendo default-location da [...]ToDB que esta 
					// sendo chamada e ela efetua apenas o WINDOW.CLOSE() e OPENER.RELOAD()
					// --------------------------------------------------------------------------------------------
				    document.formeditor_000.DEFAULT_LOCATION.value = strLocation; 
					if(window.opener != null && window.opener != "undefined") { 
					  document.formeditor_000.DEFAULT_LOCATION.value = '../_database/athclosedlg.php'; 
					}
		...
		function setFormField(formname, fieldname, valor){
			if ((formname != "") && (fieldname != "") && (valor != "")){
				...
				eval("document." + formname + "." + fieldname).dispatchEvent(event);
			}
		}
		...
		function submeterFormRel(prGrpForm, prCodGrid){
			...
			eval("document.formeditor_" + prGrpForm).DEFAULT_LOCATION.value = "../_fontes/<?php echo(getsession($strSesPfx . "_grid_default")."?var_basename".getsession($strSesPfx . "_dir_modulo")."&var_cod_pai=".$intCodDado); ?>&var_dlg_grp=" + prGrpForm + "&var_cod_dialog_grid=" + prCodGrid;
	...
	<?php if(getsession($strSesPfx . "_field_detail") == "") {?> background="../img/bgFrame_<?php echo(CFG_SYSTEM_THEME); ?>_main.jpg" <?php } ?>
	<?php if(getsession($strSesPfx . "_js_onload") != "")    {?> onload="<?php echo(getsession($strSesPfx . "_js_onload")); ?>" <?php } ?>
	>
	...
	<tr>
		<td valign="top">
			...
			<table id="dialog_cont_<?php echo(getValue($objRS,"dlg_grp")); ?>" class="kernel_dialog" width="100%" cellpadding="0" cellspacing="0" style="<?php echo($strCollapse); ?>">
				...
				<input type="hidden" name="var_return" value="<?php echo($strReturnOk);?>">
				...
				<?php
						...
						if($strOperacao != "INS" && !getValue($objRS,"valor_sistema") && $strDialogGrp == "000"){
							...
							//echo getsession(getValue($objRS,"valor_padrao"));
						...
						if(!$boolContinua && ($strDialogGrp == "000" || ($strOperacao != "VIE" && $strOperacao != "DEL"))){
								...
								case "FILE_IMAGE":
									//if ($strValor!=''){
										...
										if(preg_match("/\.(jp[e]?g|gif|png)$/",$strValor)) {
											 $strAuxPathView = strtolower(str_replace("\\","/",replaceParametersSession(getValue($objRS,"file_dir_arquivos"))));
											 $strPathView = findLogicalPath($strAuxPathView) . "/" . $strValor;
											...
											*/
											$intTamanho = 150;
											//if (file_exists($strPathView)){
											
											//}//else {$strValor = "";}
											
										}
										if (false!==file($strPathView)){
											echo "<img id	='".$strComponente."_vw' src=\"" . $strPathView . "\" border=\"0\" width=\"" . $intTamanho . "\"><br><br>";
										}else{
											echo "<img id	='".$strComponente."_vw' src='https://tradeunion.proevento.com.br/_tradeunion/img/blank_img.png' border=\"0\" width=\"" . $intTamanho . "\"><br><br>";
										}
									...
									echo("<input type='hidden' name='".$strComponente."' id='".$strComponente."_".getValue($objRS,"dlg_grp")."' "); 
									echo(" value='".$strValor."'  title='".getTText($strNomeField,C_NONE). "' ");
									echo(" " . $strChamadas);
									echo(" style='width:80px;' >");
									echo("<input type='button' name='btn_uploader' value='Upload' class='inputcleanActionOk'  ");
									
									echo("onClick=\"document.getElementById('".$strComponente."_".getValue($objRS,"dlg_grp")."').readOnly=false;callUploader('formeditor_" . getValue($objRS,"dlg_grp") . "','" . $strComponente ."','\\\\" . str_replace("\\","\\\\",replaceParametersSession(getValue($objRS,"file_dir_arquivos"))) . "\\\\','" . replaceParametersSession(getValue($objRS,"file_prefix")) . "','" . getValue($objRS,"file_flag_sufix") . "');\">");
									if ($strValor!=''){
										...
										if(!preg_match("/\.(jp[e]?g|gif|png)$/",$strValor)) {
											$linkFile = "   <a href=\"" . $strPath . "\" target=\"_blank\">" . $strValor . "</a>";
										}
										}
								break;//*/
								...
								case "LABEL":
									if($strValor != "") {
										...
										if(getValue($objRS,"classe") == "FILE") {
											...
											$strValor = "<a href=\"" . $strPath . "\" target=\"_blank\">" . $strValor . "</a>";
										}
							...
							if ($strOperacao == "INS" || $strOperacao == "UPD") {
								...
								echo("&nbsp;" . getValue($objRS,"descricao",false) . "<br>".$linkFile."</span>&nbsp;</td>\n");
								$linkFile = "";	
							}//*/
						...
						if($strDialogGrp != getValue($objRS,"dlg_grp")) {
							...
							if($strDialogGrp != "000") {
								if(getValue($objRSGrid,"cod_dialog_grid") != "") {
									$strIframeDialogSrc = (getValue($objRSGrid,"grid_default") != "") ? getValue($objRSGrid,"grid_default") : "../_fontes/data.php";
									$strIframeDialogSrc .= "?var_cod_pai=" . $intCodDado . "&var_cod_dialog_grid=" . getValue($objRSGrid,"cod_dialog_grid");
									$strIframeDialogSrc .= "&var_basename=".$strBaseName;

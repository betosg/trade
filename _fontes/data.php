...
if($strAcao == "") {
	...
	foreach($objResult2 as $objRS2) {
		if(getValue($objRS2,"link_img") != ""){
			...
			if (strpos($arrIconConf[$intContIcons]["link"],"var_basename") === false){
			    ...
			    if($strAcao == "" && getsession($strSesPfx . "_menucombo_rotulo") != "" && getsession($strSesPfx . "_menucombo_valores") != "")	{ 
					...
					switch($strTpMenu) {
						...
						case 'BUTTOM':	while($intI < count($arrMenuRotulo)){
											$strLink = trim($arrMenuValor[$intI]);
											if($arrMenuValor[$intI] != NULL){
												echo("<button onclick=\"javascript:aplicarFuncao('".$strLink."');\" class='inputcleanActionOk'>" . getTText(trim($arrMenuRotulo[$intI]),C_NONE) . "</button>\n");
											}
											$intI++;
										}
										break;
						case 'BUTTOM_POPUP':
										while($intI < count($arrMenuRotulo)){
											$strLink = trim($arrMenuValor[$intI]);
											if($arrMenuValor[$intI] != NULL){
												echo("<button onclick=\"javascript:".$strLink."\" class='inputcleanActionOk'>" . getTText(trim($arrMenuRotulo[$intI]),C_NONE) . "</button>\n");
											}
											$intI++;
										}
										break;
				...
				<table border="0" cellpadding="0" cellspacing="0">
				  <form name="formpaginacao" action="data.php" method="post">
					...
					<tr>
						...
						<td style="padding:0px 10px 0px 10px;"><?php echo(getTText("pagina",C_TOLOWER)); ?> <input type="text" name="var_curpage" value="<?php echo($intNumCurPage)?>" size="3"></td>

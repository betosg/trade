...
<form name="formeditor_000" action="dataprepare.php" method="post" target="<?php echo(CFG_SYSTEM_NAME . "_main"); ?>">
 <input type="hidden" name="var_basename" id="var_basename" value="<?php echo($strBaseName) ?>" />
<?php
	$strValorPadrao = ""; 
	$strRotuloGRP = "";
	$strSetDisabled = "";
	...
	foreach($objResult as $objRS){
		// echo("[ ".getValue($objRS,"valor_padrao")." ]");
		// echo("{ ".getValue($objRS,"valor_sistema")." }");
		...
		if($boolContinua == false){
			...
			switch($strClasse){
				...
				/****************** EDITs *******************/
				case "EDIT":
					echo("<input type='".getValue($objRS,"param_edit_type")."' value='".$strValorPadrao."' ".$strSetDisabled);
					echo("   name='" . $strComponente . "' id='" . $strComponente ) . "' "; 
			...
			echo("&nbsp;" . getValue($objRS,"descricao",false) . "</span>&nbsp;");	
...
?>  
	...
	<tr align="right" valign="middle">
	  <td><br><button onClick="submeterForm();" class='inputcleanActionOk'><?php echo(getTText("aplicar",C_UCWORDS)) ?></button></td>
	</tr>
</table>
<?php
}else{
	...
	echo("<form name='formeditor_000' action='dataprepare.php' method='post' target='". CFG_SYSTEM_NAME . "_main' >");
	echo("<input type='hidden' name='var_basename' id='var_basename'  value='" . $strBaseName . "' >");
	echo("</form>");
}
athEndFloatingBox();

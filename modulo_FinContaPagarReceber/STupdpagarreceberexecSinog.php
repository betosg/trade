...
//$dblVlrMoraMulta	= trim(request("var_vlr_mora_multa")); //Não é mais alterado por aqui, calculado em outra página
//$dblVlrOutrosAcresc	= trim(request("var_vlr_outros_acresc")); //Não é mais alterado por aqui, calculado em outra página
...
$strArquivo			= trim(request("dbvar_str_arquivo_1"));
...
if($intCodDado != "" && $intCodConta != "" && $strTipo != "" && $intCodigo != "" && $intCodCentroCusto != "" && $intCodPlanoConta != "" && $strTipoDocumento != "" && $dateDtEmissao != "" && $dateDtVctoOrig != "" && $strHistorico != ""){
	...
	//if ($dblVlrMoraMulta != "")    $dblVlrMoraMulta    = str_replace(",",".",str_replace(".","",$dblVlrMoraMulta)); 
	//if ($dblVlrOutrosAcresc != "") $dblVlrOutrosAcresc = str_replace(",",".",str_replace(".","",$dblVlrOutrosAcresc)); 
	...
	try{
		...
		//if ($dblVlrMoraMulta != "") $strSQL .= ", vlr_mora_multa = " . $dblVlrMoraMulta;
		//if ($dblVlrOutrosAcresc != "") $strSQL .= ", vlr_outros_acresc = " . $dblVlrOutrosAcresc;
	...
	$strLocation = ($strButtonAction  == "aplicar") ? "STupdpagarreceberSinog.php?var_chavereg=" . $intCodDado : $strAuxGridDefault;

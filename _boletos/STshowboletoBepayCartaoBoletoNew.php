...
$strTipo             = $strRequest[0];
$strNomeCartao       = $strRequest[1];
$strCPFCartao        = $strRequest[2];
$strNumCartao        = $strRequest[3];
$strCvvCartao        = $strRequest[4];
$strMesCartao        = $strRequest[5];
$strAnoCartao        = $strRequest[6];
$strBandeira         = $strRequest[7];
$intCodDado          = $strRequest[8];
$intParcela          = $strRequest[9];
        ...
        if (trim($resultado["data"]["financialStatement"]["status"])=="APPROVED") {
			...
			try{
					$strSQL = "INSERT INTO fin_conta_pagar_receber_log_bepay 
							...
							values  ('".$DadosBoleto_SACADO_NOME."','".$DadosBoleto_SACADO_EMAIL."',".$intCodDado.", '".$data_post."', '" . $result . "', '".getsession(CFG_SYSTEM_NAME . "_id_usuario")."',now(), null,'".$strIdentificador."', 1) ";
					$objResult = $objConn->query($strSQL);
					$objConn->commit();
			}catch(PDOException $e){
				mensagem("err_sql_titulo","err_sql_desc",$e->getMessage(),"","erro",1);
...
try{
		$strSQL = "INSERT INTO fin_conta_pagar_receber_log_bepay 
						(nome_sacado,email_sacado, cod_conta_pagar_receber, data_post,	return_post, sys_usr_ins, sys_dtt_ins, transaction_id, identificador, cartao_parcela)
				values  ('".$DadosBoleto_SACADO_NOME."','".$DadosBoleto_SACADO_EMAIL."',".$intCodDado.", '".$data_post."', '" . $result . "', '".getsession(CFG_SYSTEM_NAME . "_id_usuario")."',now(), '".$transactionId."','".$strIdentificador."', ".$intParcela.") ";
		$objResult = $objConn->query($strSQL);
		$objConn->commit();
}catch(PDOException $e){
	mensagem("err_sql_titulo","err_sql_desc",$e->getMessage(),"","erro",1);
	...
	die();
			...
			//var_dump(get_resource_type($ch));
						if ($strTipo == "boleto"){
                            ...
                            try{
                                  //  $strSQL = "INSERT INTO fin_conta_pagar_receber_log_bepay 
                                  //                  (nome_sacado,email_sacado, cod_conta_pagar_receber, data_post,	return_post, sys_usr_ins, sys_dtt_ins, transaction_id, identificador)
                                  //          values  ('".$DadosBoleto_SACADO_NOME."','".$DadosBoleto_SACADO_EMAIL."',".$intCodDado.", '".$data_post."', '" . $result . "', '".getsession(CFG_SYSTEM_NAME . "_id_usuario")."',now(), '".$transactionId."','".$strIdentificador."') ";
                                  //  $objResult = $objConn->query($strSQL);
                                    ...
                                    $strSQL .= "                     VALUES (" . $intCodDado . "        , '" . $strTipo . "', " . $intCodigo . ", " . $intCodConta . ", " . $intCodPlanoConta . ", " . $intCodCentroCusto . ", " . $intCodJob . ", '" . $strHistorico . "', '" . $strObs . "', '" . $strNumLcto . "', current_date, "   . $totalPago . ", " . $dblVlrMulta . ", " . $dblVlrJuros . ", " . $dblVlrDesc . ", CURRENT_TIMESTAMP, '" . $strSysUsuario . "', '" . $strDocumento . "'     , '" . $numDocumento . "', '" . $strExtraDocumento . "') ";
                                    $objConn->query($strSQL);
                        
                                    $strSQL  = " UPDATE fin_conta_pagar_receber ";
                                    $strSQL .= " SET sys_dtt_ult_lcto = CURRENT_TIMESTAMP , cod_job =  " . $intCodJob;
...
/*	=======================================================
					...
					BUSCA DADOS EXTRATO 
						...
						$headers[] = "Transaction-Hash: ".trim($AuthorizationHeaderBase64);
							...
							$resultadoExtrato="";
							$resultadoExtrato=array();	
							$resultadoExtrato = json_decode($resultExtrato,true);
							$arrDadosExtrato = $resultadoExtrato["data"]["statement"];
							
	
							
                            $strNomeExtrato = "";
							foreach($arrDadosExtrato as $key => $value) {
                                if ($arrDadosExtrato[$key]["transactionId"] == $transactionId){
									//if ($arrDadosExtrato[$key]["transactionId"] == "330886BA-CD1D-B9CF-B470-B02778DED6FE"){
                                    if ($strNomeExtrato == ''){$strNomeExtrato = $arrDadosExtrato[$key]["counterpart"]["name"];}
                                    $arrNovoLcto[] = ($arrDadosExtrato[$key]);
                                    ...
                                    $strSQLBusca  .= "                                                                FROM  fin_conta_pagar_receber_log_bepay where transaction_id = '".$arrDadosExtrato[$key]["transactionId"]."')";
                                    ...
                                    $totalPagoExtrato = $arrDadosExtrato[$key]["amount"];

                                    $strObsExtrato            = "Lançamento automatico Transaction: ".$arrDadosExtrato[$key]["transactionId"];
                                    $strDocumentoExtrato  = "CARTAO";
                                    $strNumLctoExtrato = "TITULO: ". getValue($objRS,"cod_conta_pagar_receber");
									//$dblVlrLctoNorm =  formatcurrency($totalPago,2); 
                                    ...
                                    $creditDateExtrato = date_create($arrDadosExtrato[$key]["creditDate"]);                                    
                                    $creditDateExtrato =  date_format($creditDateExtrato,'d/m/Y');
									$idTransaction =$arrDadosExtrato[$key]["transactionId"];
									
                                    if ($arrDadosExtrato[$key]["type"] == 'C'){
                                       // print("<BR>OPERAÇÃO CREDITO<BR>");
                                       // print("<pre>");
                                       //     print_r($arrDadosExtrato[$key]);
                                        //print("</pre><BR>");
										...
										$strAdicionalInfoParcela = "Parc.: " . $arrDadosExtrato[$key]["additionalInfo"];
										$sumTotalPago = $totalPagoExtrato + $sumTotalPago;
                                    }

                                    $strHistoricoDespesa2 = "";
                                    $strSQLDesp = "";
                                    if ($arrDadosExtrato[$key]["type"] == 'D'){
                                        /**Lançamento Despesa */
                                        $codContaDespesa = 29;
										...
										$codJobDespesa = 61;
										$strHistoricoDespesa2 = "TAXA BEPAY CARTAO - ". $strNomeExtrato ." (".$intCodDadoExtrato.") - " . $strAdicionalInfoParcela;
										if ($arrDadosExtrato[$key]["description"] == "PGTO SERV MEDIADOR"){
											$strHistoricoDespesa2 = "TAXA BEPAY CARTÃO MDR - ". $strNomeExtrato ." (".$intCodDadoExtrato.")" ;
										}
                         ...
                         if ($sumTotalPago == $strValorConta){
							$objConn->beginTransaction();
							try{
								$arrSQLQuitacao  =  explode("|",$strSQLQuitacao);									
									for ($i=1;$i< count($arrSQLQuitacao);$i++){
										echo("<br>".$arrSQLQuitacao[$i]);
										echo $arrSQLQuitacao[$i];
										$objConn->query($arrSQLQuitacao[$i]);
									}
									$strSQL  = " UPDATE fin_conta_pagar_receber ";
                                    $strSQL .= " SET sys_dtt_ult_lcto = CURRENT_TIMESTAMP , cod_job =  " . $intCodJob;
                                    $strSQL .= "   , sys_usr_ult_lcto = '" . getsession(CFG_SYSTEM_NAME . "_id_usuario") . "' ";

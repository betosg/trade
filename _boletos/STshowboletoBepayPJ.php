...
try{
 $strSQL = " SELECT
					...
					,  CASE WHEN fin_conta_pagar_receber.tipo = 'cad_pf' THEN (SELECT email FROM cad_pf WHERE cod_pf = fin_conta_pagar_receber.codigo)
						   WHEN fin_conta_pagar_receber.tipo = 'cad_pj' THEN (SELECT email FROM cad_pj WHERE cod_pj = fin_conta_pagar_receber.codigo)
						   WHEN fin_conta_pagar_receber.tipo = 'cad_pj_fornec' THEN (SELECT email FROM cad_pj_fornec WHERE cod_pj_fornec = fin_conta_pagar_receber.codigo)
					...
					,  CASE WHEN fin_conta_pagar_receber.tipo = 'cad_pf' THEN (SELECT endprin_fone2 FROM cad_pf WHERE cod_pf = fin_conta_pagar_receber.codigo)
						   WHEN fin_conta_pagar_receber.tipo = 'cad_pj' THEN (SELECT endprin_fone2 FROM cad_pj WHERE cod_pj = fin_conta_pagar_receber.codigo)
						   WHEN fin_conta_pagar_receber.tipo = 'cad_pj_fornec' THEN (SELECT endprin_fone2 FROM cad_pj_fornec WHERE cod_pj_fornec = fin_conta_pagar_receber.codigo)
...
$strIdentificador = "10812688000168ABFM".getValue($objRS,"sequecial_boleto").getValue($objRS,"cod_pj").$intCodDado;
...
if (!phoneValidate($strCelular)){
	$strMsg .=  " - CELULAR inválido.( O número deve ser composto por 11 98888-7777.)";
}
...
if ($strMsg !=""){ ?>
	<script language="javascript">
		<?php if (!isset($_GET["var_basename"])){?>
			alert('<?php echo("Atualize seus dados cadastrais, e verifique os campos abaixo para emissão do seu boleto.\\r\\n".$strMsg);?>');		
			window.parent.document.location.href = "_fontes/insupddelmastereditor.php?var_oper=UPD&var_chavereg=<?php echo(getValue($objRS,"cod_pj"));?>&var_basename=modulo_CadPJ"
			//https://tradeunion.proevento.com.br/_tradeunion/
		<?php } else {?>
			alert('<?php echo("Acesse o cadastro da empresa ".$DadosBoleto_SACADO_NOME." (".getValue($objRS,"cod_pj")."). \\r\\n Atualize os dados cadastrais, e verifique os campos abaixo para emissão do boleto.\\r\\n".$strMsg);?>');		
			history.go(-1);	 
	    <?php } ?>
		//
		//history.go(-1);
...
$arrRecipients[]= array(
	...
	"amount"        => utf8_encode($DadosBoleto_VLR_TITULO),
	"mediatorFee"   => utf8_encode("2.01"),

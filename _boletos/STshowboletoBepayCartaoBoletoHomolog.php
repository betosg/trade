...
//	die();
    ...
    if ($strTipo == "cartao"){
        //print_r($resultado);
        if (trim($resultado["data"]["financialStatement"]["status"])=="REJECTED") {
           ...
           // $totalPago       = $resultado['data']['totalAmount'];
			...
			try{
					$strSQL = "INSERT INTO fin_conta_pagar_receber_log_bepay 
							...
							values  ('".$DadosBoleto_SACADO_NOME."','".$DadosBoleto_SACADO_EMAIL."',".$intCodDado.", '".$data_post."', '" . $result . "', '".getsession(CFG_SYSTEM_NAME . "_id_usuario")."',now(), null,'".$strIdentificador."') ";
					$objResult = $objConn->query($strSQL);
					$objConn->commit();
			}catch(PDOException $e){
				mensagem("err_sql_titulo","err_sql_desc",$e->getMessage(),"","erro",1);
						...
						if ($strTipo == "boleto"){
                            ...
                            try{
                                  ...
                                  //  $objResult = $objConn->query($strSQL);
									...
									//if (getValue($objRS,"cod_job")==""){
										$intCodJob         = "61";//getValue($objRS,"cod_job");
									//}else{

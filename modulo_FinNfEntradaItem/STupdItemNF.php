...
<?php
	...
	try{
		 $strSQL = "SELECT 
					...
					WHERE cod_nf_entrada_item = ".$intCodDado;
	...
	$dblQtde = number_format((double) getValue($objRS,"quantidade"), 2);
	$dblQtde = str_replace(",", "", $dblQtde);
	$dblQtde = str_replace(".", ",", $dblQtde);
	
	$dblVlrIPI = number_format((double) getValue($objRS,"vlr_ipi"), 2);
	$dblVlrIPI = str_replace(",", "", $dblVlrIPI);
	$dblVlrIPI = str_replace(".", ",", $dblVlrIPI);
	
	$dblVlrICMS = number_format((double) getValue($objRS,"vlr_icms"), 2);
	$dblVlrICMS = str_replace(",", "", $dblVlrICMS);
	$dblVlrICMS = str_replace(".", ",", $dblVlrICMS);
	
	$dblVlrUnit = number_format((double) getValue($objRS,"vlr_unitario"), 2);
	$dblVlrUnit = str_replace(",", "", $dblVlrUnit);
	$dblVlrUnit = str_replace(".", ",", $dblVlrUnit);
	
	$dblVlrTotal = number_format((double) getValue($objRS,"vlr_total"), 2);
	$dblVlrTotal = str_replace(",", "", $dblVlrTotal);
	$dblVlrTotal = str_replace(".", ",", $dblVlrTotal);
?>
<html>
	<head>
		...
		<script language="javascript" type="text/javascript">
			...
			function ok() {			
					calcula();
					strLocation = "../modulo_FinNfEntradaItem/STitemNF.php?var_codnfpai=<?php echo $intCodNfPai; ?>";
					submeterForm();
			}
			...
			function aplicar() {
				calcula();
				strLocation = "../modulo_FinNfEntradaItem/STupdItemNF.php?var_chavereg=<?php echo $intCodDado; ?>&var_codnfpai=<?php echo $intCodNfPai; ?>";
				submeterForm();
			}
...
<table cellpadding="0" cellspacing="0" border="0" width="100%">
	...
	<form name="formstatic" action="../_database/athupdatetodb.php" method="POST" />
	...
	<input type="hidden" name="DBVAR_AUTODATE_SYS_DTT_UPD" value="" />	
			...
			<td align="left" valign="top" style="padding:10px 50px 0px 50px;">
				<table cellspacing="2" cellpadding="3" border="0" width="100%">
					...
					<tr bgcolor="<?php echo(getLineColor($strColor));?>">
						...
						<td align="left" colspan="3"><?php echo $intCodNfPai;?><span class="comment_peq">&nbsp;<?php echo(getTText("obs_ins_nf_item",C_NONE));?></span></td>
					</tr>    				
					<tr bgcolor="<?php echo(getLineColor($strColor));?>">
						<td width="13%" align="right"><strong><?php echo(getTText("desc_prod",C_UCWORDS));?>:</strong></td>
						<td align="left" colspan="3"><input class="campos" name="DBVAR_STR_DESCR_PRODUTO" id="DBVAR_STR_DESCR_PRODUTO" style="width:100%" type="text" maxlength="255" value="<?php echo getValue($objRS,"descr_produto");?>"></td>
					</tr>
    				<tr bgcolor="<?php echo(getLineColor($strColor));?>">
						...
						<option value=""></option>
							<?php echo montaCombo($objConn, "SELECT cod_plano_conta, cod_reduzido || ' ' || nome AS descr_plano
															 FROM fin_plano_conta 
															 WHERE dtt_inativo IS NULL AND (cod_reduzido || ' ' || nome) IS NOT NULL
															 ORDER BY nivel, ordem, nome ", "cod_plano_conta","descr_plano",getValue($objRS,"cod_plano_conta")); ?>
						</select>
						</td>
					</tr>
					<!--
    				...
    				<tr bgcolor="<?php echo(getLineColor($strColor));?>">
						...
						<option value=""></option>
							<?php //echo montaCombo($objConn, "SELECT cod_centro_custo, cod_reduzido || ' ' || nome AS descr_plano FROM fin_centro_custo WHERE dtt_inativo IS NULL AND (cod_reduzido || ' ' || nome) IS NOT NULL ORDER BY nivel, ordem, nome ", "cod_centro_custo","descr_plano",getValue($objRS,"cod_centro_custo")); ?>
						</select>
						</td>
					</tr>
					-->
    				<tr bgcolor="<?php echo(getLineColor($strColor));?>">
						...
						<option value=""></option>
							<?php echo montaCombo($objConn, "SELECT cod_job, nome AS descr_plano
															 ...
															 ORDER BY ordem, nome ", "cod_job","descr_plano",getValue($objRS,"cod_job")); ?>
						</select>
						</td>
					</tr>
					<tr bgcolor="<?php echo(getLineColor($strColor));?>">
						<td width="13%" align="right"><strong><?php echo(getTText("quantidade",C_UCWORDS));?>:</strong></td>
						<td width="25%" align="left"> <input type="text" name="DBVAR_MOEDA_QUANTIDADE" id="DBVAR_MOEDA_QUANTIDADE" size="7" maxlength="7" onKeyPress="Javascript:return validateFloatKeyNew(this, event);" onBlur="calcula();" value="<?php echo $dblQtde;?>" /></td>
						<td width="21%" align="right"><strong><?php echo(getTText("unidade",C_UCWORDS));?>:</strong></td>
						<td width="41%" align="left"> <input type="text" name="DBVAR_STR_UNIDADE" id="DBVAR_STR_UNIDADE"  size="15" maxlength="2" value="<?php echo getValue($objRS,"unidade");?>" /></td>						
					</tr>
					<tr bgcolor="<?php echo(getLineColor($strColor));?>">
						<td width="13%" align="right"><strong><?php echo(getTText("ipi",C_UCWORDS));?>:</strong></td>
						<td align="left"><input type="text" name="DBVAR_MOEDA_VLR_IPI" id="DBVAR_MOEDA_VLR_IPI" onKeyPress="Javascript:return validateFloatKeyNew(this, event);" size="15" maxlength="11" value="<?php echo $dblVlrIPI;?>" /></td>
					    <td align="right"><strong><?php echo(getTText("icms",C_UCWORDS));?>:</strong></td>
					    <td align="left"><input type="text" name="DBVAR_MOEDA_VLR_ICMS" id="DBVAR_MOEDA_VLR_ICMS" onKeyPress="Javascript:return validateFloatKeyNew(this, event);" size="15" maxlength="11" value="<?php echo $dblVlrICMS; ?>" /></td>
					</tr>					
					<tr bgcolor="<?php echo(getLineColor($strColor));?>">
						<td width="13%" align="right"><strong><?php echo(getTText("preco_final",C_UCWORDS));?>:</strong></td>
					    <td width="25%" align="left"><input type="text" name="DBVAR_MOEDA_VLR_UNITARIO" id="DBVAR_MOEDA_VLR_UNITARIO" onKeyPress="Javascript:return validateFloatKeyNew(this, event, 'yes');" onBlur="calcula();" size="15" maxlength="19" value="<?php echo $dblVlrUnit; ?>" /></td>
						<td width="21%" align="right"><strong><?php echo(getTText("sub_total",C_UCWORDS));?>:</strong></td>
						<td width="41%" align="left"><strong><input type="text" name="DBVAR_MOEDA_VLR_TOTAL" id="DBVAR_MOEDA_VLR_TOTAL" size="20" readonly="true" style="border:none; text-decoration:blink" value="<?php echo $dblVlrTotal; ?>" /></strong></td>																					
					  <!-- FIM CAMPOS -->

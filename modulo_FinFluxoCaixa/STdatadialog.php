...
$intCodDialogGrid = request("var_cod_dialog_grid");
$intCodDado 	  = request("var_cod_pai");
...
$strSQLGrid = str_replace(";","",str_replace("{*}",$intCodDado,getValue($objRSGrid,"grid_query")));
...
try{
	...
	if(getValue($objRSGrid,"num_per_page") != ""){
		...
		$objRSCount  = $objConn->query($strSQLCount)->fetchAll();
		$intTotalRegistros = $objRSCount[0]["total"];
...
if($strAcao == ""){
	...
	try{
		$strSQL     = " SELECT nome, link, link_img, rotulo
...
<script>
	function setOrderBy(prStrOrder,prStrDirect){
		location.href = "STdatadialog.php?var_cod_pai=<?php echo($intCodDado); ?>&var_cod_dialog_grid=<?php echo($intCodDialogGrid); ?>&var_order_column=" + prStrOrder + "&var_order_direct=" + prStrDirect;
	}
							...
							<table id="tableContent" border="0" cellpadding="0" cellspacing="0" width="100%" background="../img/grid_backheader.gif" style="background-repeat:repeat-x;">
								<tr>
									<?php
										...
										//if($intContIcons > 0 && $strAcao == ""){ //Coloca uma coluna mesclada para ajustar a tabela com os ícones que virão abaixo
											echo("<td></td>");
										//}
			...
			<td align="right">
				<table border="0" cellpadding="0" cellspacing="0">
				  <form name="formpaginacao" action="STdatadialog.php" method="post">
					...
					<input type="hidden" name="var_cod_pai" value="<?php echo($intCodDado); ?>">
					<tr>

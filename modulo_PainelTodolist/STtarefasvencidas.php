<?php
	...
	try{
		...
		$strSQLVencidas .= (isset($strTipoUsuario) && ($strTipoUsuario) == "todos") ? " AND (id_responsavel = '".getsession(CFG_SYSTEM_NAME."_id_usuario")."' OR id_ult_executor = '".getsession(CFG_SYSTEM_NAME."_id_usuario")."')" : "";
		$strSQLVencidas .= " ORDER BY prev_dt_ini ASC, situacao ";
		// echo($strSQL);
		$objResultVencidas = $objConn->query($strSQLVencidas);
		$objConn->commit();
...
<div id="grupo_1" style="display:block;">
	...
	<tr>
		<td align="center" valign="top">
			<!-- FOREACH DADOS -->
			<?php if($objResultVencidas->rowCount() > 0){?>
				<?php foreach($objResultVencidas as $objRSVenc){?>
				<div style="width:100%;background-color:#FFB6C1;border:1px solid #CCC;margin-top:5px;cursor:default;">
					<div style="float:left;width:15px;padding:5px;">
						<?php echo(getValue($objRSVenc,"situacao_grid"));?><br />
						<?php echo(getValue($objRSVenc,"prioridade_grid"));?>
					</div>
					<div style="width:150px;height:100%;overflow:hidden;text-align:left;padding-top:7px;">
						<strong><?php echo(getValue($objRSVenc,"titulo"))?></strong>
						<br/>
						<?php echo("(".dDate(CFG_LANG,getValue($objRSVenc,"prev_dt_ini"),false)." ".getValue($objRSVenc,"prev_hr_ini").") ");?>
					</div>
				</div>
				<?php }?>
			<?php } else{?>

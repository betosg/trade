	...
	<head>
		...
		<script language="javascript" type="text/javascript">
			...
			function submeterForm() {
				...
				<?php if (strpos(CFG_DB, "abramge") === false) { ?>
				if(document.getElementById("abramge").checked){
					var_destinos += '|' + document.getElementById("abramge").value
				}
				<?php } if (strpos(CFG_DB, "sinamge") === false) { ?>
				if(document.getElementById("sinamge").checked){
					var_destinos += '|' + document.getElementById("sinamge").value;
				}
				<?php } if (strpos(CFG_DB, "sinog") === false) { ?>
				if(document.getElementById("sinog").checked){					
					var_destinos += '|' + document.getElementById("sinog").value
				}
				<?php } if (strpos(CFG_DB, "uca") === false) { ?>
				if(document.getElementById("uca").checked){
					var_destinos += '|' + document.getElementById("uca").value
				}
			<?php }?>
				alert(var_destinos)
				//return false;
...
<table cellpadding="0" cellspacing="0" border="0" width="100%">
	<tr>
	<td align="center">
	<?php athBeginFloatingBox("520","none",getTText("replica_cadastro_title",C_NONE),CL_CORBAR_GLASS_1); ?>
	<form name="formstatic" action="#" method="POST" onSubmit=""  />
        
        <input type="hidden" name="DEFAULT_DB" 				value="<?php echo(CFG_DB);?>" />
        <input type="hidden" name="var_destinos" 			value="DBVAR_" />
                    ...
                    <table cellspacing="2" cellpadding="3" border="0" width="100%">
                        ...
                        <tr bgcolor="<?php echo(getLineColor($strColor));?>">
                            ...
                            <td width="73%" align="left">
								<?php echo(getTText("instrucao_selecao",C_UCWORDS));?>
							</td>
						</tr>				
						<?php if (strpos(CFG_DB, "abramge") === false) { ?>
						<tr bgcolor="<?php echo(getLineColor($strColor));?>">
                            <td width="27%" align="right"></td>
                            <td width="73%" align="left">
								<input type="checkbox" name="ambiente" id="abramge" value="abramge" >&nbsp;ABRAMGE
							</td>
						</tr>
						...
						<tr bgcolor="<?php echo(getLineColor($strColor));?>">
                            <td width="27%" align="right"></td>
                            <td width="73%" align="left">
								<input type="checkbox" name="ambiente" id="sinamge" value="sinamge"  >&nbsp;Sinamge
							</td>
						</tr>              
						...
						<tr bgcolor="<?php echo(getLineColor($strColor));?>">
                            <td width="27%" align="right"></td>
                            <td width="73%" align="left">
								<input type="checkbox" name="ambiente" id="sinog"  value="sinog" >&nbsp;SINOG
							</td>
						</tr>              
						...
						<tr bgcolor="<?php echo(getLineColor($strColor));?>">
                            <td width="27%" align="right"></td>
                            <td width="73%" align="left">
								<input type="checkbox" name="ambiente" id="uca"    value="uca" > &nbsp;UCA
							</td>
						</tr>   
						<?php }?>                     

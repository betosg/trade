...
$dbOrigem    = request("DEFAULT_DB");
$strDestinos = request("var_destinos");
...
$objConnOrigem = abreDbConn($dbOrigem);
...
try {
	...
	//die($strSQL);
	$objConnOrigem->query($strSQL);

		$comando = $objConnOrigem->prepare($strSQL);
		$comando->execute();
...
try {
	...
	$strSQL  = " SELECT cod_pj FROM cad_pj where cnpj = '". $cnpj ."'";
	
	if (strpos($strDestinos, "sinog") !== false) {
		$objConnAbramge->query($strSQL);
		$comando = $objConnAbramge->prepare($strSQL);
		$comando->execute();			
		while($row = $comando->fetch(PDO::FETCH_OBJ)){
			$intCodPjBuscaAbramge .= $row->cod_pj;
		}
		echo("<br> Achou ou nao? ".$intCodPjBuscaAbramge);
	}
	if (strpos($strDestinos, "sinog") !== false) {	
...
echo "<br> busca: ".$intCodPjBuscaAbramge;
*/
...
if (strpos($strDestinos, "sinamge") !== false) {
...
if (strpos($strDestinos, "uca") !== false) {
		if ($intCodPjBuscaUca == ""){
...
if (strpos($strDestinos, "abramge") !== false) {
	if ($intCodPjBuscaAbramge == ""){
		$strSQLCadPjAbramge = $strSQLIns;
	}else{
		$strSQLCadPjAbramge = $strSQLUpd . " " . $intCodPjBuscaAbramge;
	}
	$objConnAbramge->query($strSQLCadPjAbramge);
}
echo "<br>Sinog: ".$strSQLCadPjSinog     ."<br>";    
echo "<br>Sinamge".$strSQLCadPjSinamge   ."<br>";   
echo "<br>Uca".$strSQLCadPjUca       ."<br>";
echo "<br>Abramge".$strSQLCadPjAbramge       ."<br>";

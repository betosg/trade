...
function GeraLinhaHeader($prCont, $prCodEmpresa, $prNomeEmpresa, $prNumBanco, $prNomeBanco, $prDia, $prMes, $prAno, $prNumSeqRemessa) {
	$strLinha = "";
	
	$strLinha .= "01REMESSA01COBRANCA";
	$strLinha .= str_pad("", 7);
	$strLinha .= str_pad($prCodEmpresa, 20, "0", STR_PAD_LEFT);
	...
	$strLinha .= str_pad("", 8);
	$strLinha .= "MX";
	$strLinha .= str_pad($prNumSeqRemessa, 7, "0", STR_PAD_LEFT);
	$strLinha .= str_pad("", 277);
	$strLinha .= str_pad($prCont, 6, "0", STR_PAD_LEFT);
	
	return $strLinha;
}

function GeraLinhaTipo1($prCont, $prIdentEmpresaCedente, $prCarteira, $prNumBanco, $prNossoNumero, $prOcorrencia, $prNumDocumento, $prDiaVcto, $prMesVcto, $prAnoVcto, $prDiaEmissao, $prMesEmissao, $prAnoEmissao, $prVlrTitulo, $prPrimMensagem, $prSacadoCNPJ, $prSacadoNome, $prSacadoEndereco, $prSacadoCEP) {
	$strLinha = "";
...
// -------------------------------------
$iCodEmpresa = getVarEntidade($objConn, "remessa_bradesco_400pos_cod_empresa");
$strNomeEmpresa = getVarEntidade($objConn, "nome_fan");
$strNumBanco = "237";
$strNomeBanco = "BRADESCO";
$iDia = date("d");
$iMes = date("m");
$iAno = date("y");
$iNumSeqRemessa = getVarEntidade($objConn, "remessa_bradesco_400pos_num_seq_arquivo");
$strCarteira = getVarEntidade($objConn, "remessa_bradesco_400pos_carteira"); //"009";  //TEMPORARIO

$strLinha = GeraLinhaHeader($iCont, $iCodEmpresa, $strNomeEmpresa, $strNumBanco, $strNomeBanco, $iDia, $iMes, $iAno, $iNumSeqRemessa);
...
try{
	$strSQL = " UPDATE sys_var_entidade
				...
				WHERE id_var ILIKE 'remessa_bradesco_400pos_num_seq_arquivo' ";
...
try{
	$strSQL = " SELECT t1.nosso_numero, t1.num_documento, t1.dt_vcto, t1.dt_emissao, t1.vlr_conta
				...
				AND t4.num_banco = '237' "; //FIXO POR ENQUANTO
...
$strIdentEmpresaCedente = getVarEntidade($objConn, "remessa_bradesco_400pos_id_empresa_cedente");
...
foreach($objResult as $objRS){
	...
	$strLinha = GeraLinhaTipo1($iCont, $strIdentEmpresaCedente, $strCarteira, $strNumBanco, $strNossoNumero, $strOcorrencia, $strNumDocumento, $iDiaVcto, $iMesVcto, $iAnoVcto, $iDiaEmissao, $iMesEmissao, $iAnoEmissao, $dblVlrTitulo, "", $strSacadoCNPJ, $strSacadoNome, $strSacadoEndereco, $strSacadoCEP);
	fwrite($fArquivo, $strLinha . chr(13) . chr(10));
	$iCont++;
}

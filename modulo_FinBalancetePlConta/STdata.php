							...
							<table id="tableContent" border="0" cellpadding="0" cellspacing="0" width="100%" background="../img/grid_backheader.gif" style="background-repeat:repeat-x;">
								...
								?>
										...
										<tr bgcolor="#EEEEEE" height="22" valign="middle">
											<td style="padding-left:20px;"><strong><?php echo(getValue($objRSConta,"nome"));?></strong></td>
											<td class="padrao_gde" align="right"><?php echo(getTText("saldo_inicial",C_NONE));?>:</td>
									...
									<?php 
											...
											foreach($objResultTitsAgrupados as $objRSAgp){
												...
												if(!isset($intCodTituloAgp[getValue($objRSAgp,"cod_agrupador")])){//$intCodTituloAgp == getValue($objRSAgp,"cod_agrupador")){
													try{
														...
														$strSQL .= " AND fin_lcto_ordinario.cod_conta = ".getValue($objRSConta,"cod_conta")." ORDER BY fin_conta_pagar_receber.cod_agrupador ";
														//die($strSQL);
										...
										try{
											$strSQL = "SELECT SUM(vlr_lcto) AS total_transf_in  FROM fin_lcto_transf WHERE dt_lcto BETWEEN '".$dtDateInicio."' AND '".$dtDateFim."' AND cod_conta_dest = ".getValue($objRSConta,"cod_conta");
											$objResultTransf = $objConn->query($strSQL);
											$objRSTransf	 = $objResultTransf->fetch();
											$dblVlrTransfIn	 = getValue($objRSTransf,"total_transf_in");
											$strSQL = "SELECT SUM(vlr_lcto) AS total_transf_out FROM fin_lcto_transf WHERE dt_lcto BETWEEN '".$dtDateInicio."' AND '".$dtDateFim."' AND cod_conta_orig = ".getValue($objRSConta,"cod_conta");
											$objResultTransf = $objConn->query($strSQL);
											$objRSTransf	 = $objResultTransf->fetch();
											$dblVlrTransfOut = getValue($objRSTransf,"total_transf_out");
										}catch(PDOException $e){
											mensagem("err_sql_titulo","err_sql_desc",$e->getMessage(),"","erro",1,"");
											die();
										}
										if(($dblVlrTransfOut > 0) || ($dblVlrTransfIn > 0)){
											$strOutput = "
												<tr bgcolor='".getLineColor($strColor)."' onMouseOver='intCurrentPosMouse = this.rowIndex;navigateRow(event);'>
													...
													<td class='field_numeric' style='text-align:right;'>".number_format((double) $dblVlrTransfIn,2,',','.')."</td>
													<td class='field_numeric' style='text-align:right;'>".number_format((double) $dblVlrTransfOut,2,',','.')."</td>
													<td style='text-align:center;'></td>
												</tr>";
											echo($strOutput);
										}
										// Soma o valor TOTAL DE TRANSFERENCIAS
										$dblTOTALGERALENTRADAPLANOSCONTA = $dblTOTALGERALENTRADAPLANOSCONTA + $dblVlrTransfIn;
										$dblTOTALGERALSAIDAPLANOSCONTA   = $dblTOTALGERALSAIDAPLANOSCONTA   + $dblVlrTransfOut;
										...
										$dblVlrTotalPlContas  = $dblParcialPlContas + getValue($objRSSldIni,"saldo_ini");
										...
										<!-- TOTAL GERADO PELA CONTA BANCO -->
										<tr bgcolor="#EEEEEE" height="22" valign="middle">
											<td style="padding-left:10px;"></td>
											<td class="padrao_gde" align="right"><?php echo(getTText("saldo_final",C_NONE));?>:</td>
											<td class="field_numeric" width="60"><strong><?php echo(number_format((double) $dblVlrTotalPlContas,2,',','.'));?></strong></td>
											<td colspan="3"></td>
										</tr>
										<tr>
								...
								<?php 
									...
									?>
										...
										<tr height="22" valign="middle">
											<td style="padding-left:10px;"></td>
											<td class="padrao_gde" align="right"><?php echo(getTText("total_geral",C_NONE));?>:</td>

...
try {
	...
	//$strSQL .= " ORDER BY 10, 1 "; Solicitação do brunet para ordenar por data BY GS
						...
						<td>
							...
							<table id="tableContent" border="0" cellpadding="0" cellspacing="0" width="100%" background="../img/grid_backheader.gif" style="background-repeat:repeat-x;">
								<tr>
									...
									<td height="22" class="titulo_grade" nowrap><?php echo(getTText("data",C_UCWORDS)); ?></td>
									<td height="22" class="titulo_grade"><?php echo(getTText("entidade",C_UCWORDS)); ?></td>
									<td height="22" class="titulo_grade"><?php echo(getTText("conta_prevista",C_UCWORDS)); ?></td>
									<td height="22" class="titulo_grade"><?php echo(getTText("conta_realizada",C_UCWORDS)); ?></td>
									<td height="22" class="titulo_grade"><?php echo(getTText("historico",C_UCWORDS)); ?></td>
									<td height="22" class="titulo_grade" align="right" nowrap><?php echo(getTText("valor_entrada",C_UCWORDS)); ?></td>
									<td height="22" class="titulo_grade" align="right" nowrap><?php echo(getTText("valor_saida",C_UCWORDS)); ?></td>
									<td height="22">&nbsp;</td>
								...
								<?php
									...
									foreach($objResultConta as $objRSConta) {
										try {
											if($strMes != "") {
												...
												$strSQL .= "  WHERE to_date('01/' || mes || '/' || ano,'DD/MM/YYYY') < to_date('01/" . $strMes . "/" . $strAno . "','DD/MM/YYYY') ";
										...
										if($objRSAc = $objResultAc->fetch()) {
											...
											try {
												...
												$dblSaldoAnterior += getValue($objRSAux,"vlr_saldo_ini");
									...
									$strTR = "<tr bgcolor=\"#EEEEEE\" height=\"22\" valign=\"middle\">
												<td colspan=\"5\"></td>
												<td class=\"padrao_gde\" align=\"right\">Saldo Total Anterior</td>";
									
									$dblTotalEntrada = doubleval($dblSaldoAnterior);
									$dblTotalSaida = 0;
									
									$strTR .= "<td class=\"field_numeric\">" . showDblNumber($dblSaldoAnterior) . "</td><td colspan=\"2\"></tr>";
									
									foreach($objResult as $objRS) { 
										ob_flush();
										echo($strTR);
										if(($strMes != "") && ($strMes != date("m",strtotime(getValue($objRS,"data"))))) {
									?>
									<tr height="22" bgcolor="#EEEEEE">
										<td colspan="6" align="right" width="98%" class="padrao_gde"><strong>Valor Total:</strong></td>
									...
									<tr height="22" bgcolor="#EEEEEE">
										<td colspan="6" align="right" width="98%" class="padrao_gde"><strong>Saldo no período:</strong></td>
										<?php
											if($dblSaldoAnterior >= 0) {
												echo("
														<td class=\"field_numeric text_green\"><strong>" . showDblNumber($dblSaldoAnterior) . "</strong></td>
														<td colspan=\"2\"></td>
													</tr>
													<tr><td colspan=\"8\" height=\"1\" class=\"line_dialog\" bgcolor=\"#EEEEEE\"></td></tr>
													<tr height=\"22\" bgcolor=\"#EEEEEE\" valign=\"middle\">
														<td colspan=\"4\"></td>
														<td class=\"padrao_gde\" align=\"right\">Saldo Total Anterior</td>
														<td class=\"field_numeric\">" . showDblNumber($dblSaldoAnterior) . "</td>
														<td colspan=\"2\"></td>
													</tr>
												");
												$dblTotalEntrada = $dblSaldoAnterior;
												$dblTotalSaida = 0;
											} else {
												echo("
										<td></td>
										<td class=\"field_numeric text_red\"><strong>" . showDblNumber($dblSaldoAnterior) . "</strong></td>
										<td></td>
									</tr>
									<tr><td colspan=\"8\" height=\"1\" class=\"line_dialog\" bgcolor=\"#EEEEEE\"></td></tr>
									<tr height=\"22\" bgcolor=\"#EEEEEE\" valign=\"middle\">
									...
									");
												$dblTotalEntrada = 0;
												$dblTotalSaida = $dblSaldoAnterior;
											}
										}
										...
										if(getValue($objRS,"operacao") != "entrada") {
											$strVlrOut = getValue($objRS,"valor");
											if (getValue($objRS,"valor_juros") != "") $strVlrOut += getValue($objRS,"valor_juros");
											if (getValue($objRS,"valor_desc") != "") $strVlrOut -= getValue($objRS,"valor_desc");
											$dblTotalSaida += $strVlrOut;
											$dblSaldoAnterior -= $strVlrOut;
										} else {
											$strVlrIn = getValue($objRS,"valor");
											if (getValue($objRS,"valor_juros") != "") $strVlrIn += getValue($objRS,"valor_juros");
											if (getValue($objRS,"valor_desc") != "") $strVlrIn -= getValue($objRS,"valor_desc");
											$dblTotalEntrada += $strVlrIn;
											$dblSaldoAnterior += $strVlrIn;
										}
										
										if(getValue($objRS,"tipo") != "" && getValue($objRS,"cod_entidade") != "") {
											try {
												$strSQL =  " SELECT ";
								...
								<tr bgcolor="<?php echo($strBgColor); ?>" onMouseOver="intCurrentPosMouse = this.rowIndex;navigateRow(event);">
									...
									<td class="field_text"><?php echo(getValue($objRS,"cod_entidade") . " - " . $strNomeEntidade); ?></td>
									<td class="field_text"><?php echo(getValue($objRS,"conta_prevista")); ?></td>
									<td class="field_text"><?php echo(getValue($objRS,"conta_realizada")); ?></td>
									<td class="field_text"><?php echo($strHistorico); ?></td>
									<td class="field_numeric"><?php echo(($strVlrIn != "") ? showDblNumber($strVlrIn) : ""); ?></td>
									<td class="field_numeric"><?php echo(($strVlrOut != "") ? showDblNumber($strVlrOut) : ""); ?></td>
									<td class="field_text">
										<?php if(!$boolIsExportation) { ?>

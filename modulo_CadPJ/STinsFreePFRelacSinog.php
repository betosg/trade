...
$objConn = abreDBConn(CFG_DB);
	...
	if ($strCodPf!="") {
		...
		if ($strCodRelac!="") {
			...
			$strMSG = "Esta PF pertence a empresa de código[$intCodPJ]. <br>Clique <b> [<a href='STverifyFreeSinog.php?var_chavereg=".$intCodPJ."&var_flag_inserir=INS_LIVRE' style='cursor:pointer;'>VOLTAR</a>] </b> ou <b>  [<a href='STupdcolabSinog.php?var_cod_pj=".$intCodPJ."&var_chavereg=".$strCodPf."'>EDITAR</a>]</b> para alterar.";
		} else {
		...
		//NOVO 
		$strMSG = "O codigo de PF não foi encontrado.<br> Clique <b> [<a href='STverifyFreeSinog.php?var_chavereg=".$intCodPJ."&var_flag_inserir=INS_LIVRE' style='cursor:pointer;'>VOLTAR</a>] </b> ou <b> [<a href='STinsFreePFSinog.php?var_chavereg=".$intCodPJ."'>NOVO</a>]</b> para cadastrar.";
	}
?> 
  ...
  <td align="center" valign="top">
	<?php athBeginFloatingBox("630","none","COLABORADOR (inserção livre/contato)",CL_CORBAR_GLASS_1); ?>
		...
		<table width="100%" bgcolor="#FFFFFF" style="border:0px #A6A6A6 solid; <?php echo($strExibeForm);?>">
	   		<form name="formeditor" action="STinsFreePFexecSinog.php" method="post">
		...
		<tr>
			<td align="right" colspan="3" style="padding:10px 30px 10px 10px;">
				...
				<button onClick="verifica('../modulo_CadPJ/STupdcolabSinog.php?var_chavereg=<?php echo($intCodPF);?>&var_cod_pj=<?php echo($intCodPJ);?>&var_cpf=<?php echo($strCPF);?>');" ><?php echo(getTText("aplicar",C_UCWORDS)); ?></button>
            </td>
		</tr>		
			</form>
		</table>

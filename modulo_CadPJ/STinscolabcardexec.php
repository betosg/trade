<?php
	...
	$intCodREL = -1; //inicializa variável usada para guardar o código da relacao PF_PJ - by Vini 12-11-2012
	...
	try{
		...
		if($flagTipoPF == "NEW_PF"){
			// INI: alteração: -----------------------------------------------------------------------------
			// Mesmo que receba a indicação "NEW_PF', o sistema passa a procurar pelo CPF indicado, 
			// caso exista não insere a PF, apenas a utiliza esta PF, criando uma nova relação.
			// --------------------------------------------------------------------------- [TAREFA 26255] --
			...
			//echo($strSQL."<br /><br />");
			if ($intCodPF != "") {
				// Se encontrou a PF, então faz update básico de dados desta PF encaminhada [COLABORADOR]
				...
				// Se não encontrou a PF, então é realmente NEW_PF
		...
		if($intCodREL < 0){ //se for menor que zero é porque não entrou nos testes acima "PJ_PF", "OLD_PF", "NEW_PF"

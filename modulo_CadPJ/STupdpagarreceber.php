   ...
   <td align="center" valign="top">
	<?php athBeginFloatingBox("600","none",$strTITULO . " - " . getTText("edicao",C_UCWORDS),CL_CORBAR_GLASS_1); ?>
		<table border="0" width="100%" bgcolor="#FFFFFF" style="border:1px #A6A6A6 solid;">
		  <form name="formconf" action="STupdpagarreceberexec.php" method="post">
					...
					<table width="550" border="0" cellspacing="0" cellpadding="4">
								...
								<table border="0px" cellpadding="0px" cellspacing="0px">
									<tr>
										<td style="padding-right:5px;" valign="middle">
										  <input name='var_codigo' value='<?php echo(getValue($objRS,"codigo")); ?>' class='edtext' type='text' maxlength='10' onKeyPress='Javascript:return validateNumKey(event);' style='width:40px;'>
										</td>
										<td valign="middle">
									...
									<tr valign="middle">
										<td style="padding-right:3px;">
											<select name="var_cod_plano_conta" class="edtext" style="width:307px;">
												<?php echo(montaCombo($objConn," SELECT	cod_plano_conta, cod_reduzido || ' ' || nome AS rotulo FROM fin_plano_conta WHERE dtt_inativo IS NULL ORDER BY cod_reduzido, ordem, nome ","cod_plano_conta","rotulo",getValue($objRS,"cod_plano_conta"))); ?>
											</select>	
										</td>
										<td>
									...
									<tr>
										<td width="120">
											<select name="var_tipo_documento" class="edtext" style="width:200px;">
												<option value="BOLETO"           	<?php if(getValue($objRS,"tipo_documento") == 'BOLETO'){ echo('selected') ; }?>><?php echo(getTText("boleto",C_TOUPPER)); ?></option>
												<option value="EXTRATO"           	<?php if(getValue($objRS,"tipo_documento") == 'EXTRATO'){ echo('selected') ; }?>><?php echo(getTText("extrato",C_TOUPPER)); ?></option>
												<option value="FATURA"         	 	<?php if(getValue($objRS,"tipo_documento") == 'FATURA'){ echo('selected') ; }?>><?php echo(getTText("fatura",C_TOUPPER)); ?></option>
												<option value="HOLERITE"          	<?php if(getValue($objRS,"tipo_documento") == 'HOLERITE'){ echo('selected') ; }?>><?php echo(getTText("holerite",C_TOUPPER)); ?></option>
												<option value="NOTA_FISCAL"    		<?php if(getValue($objRS,"tipo_documento") == 'NOTA_FISCAL'){ echo('selected') ; }?>><?php echo(getTText("nota_fiscal",C_TOUPPER)); ?></option>
												<option value="TARIFA"         	 	<?php if(getValue($objRS,"tipo_documento") == 'TARIFA'){ echo('selected') ; }?>><?php echo(getTText("tarifa",C_TOUPPER)); ?></option>
												<option value="TED"             	<?php if(getValue($objRS,"tipo_documento") == 'TED'){ echo('selected') ; }?>><?php echo(getTText("ted",C_TOUPPER)); ?></option>
												<option value="CARTAO_VISA"   	 	<?php if(getValue($objRS,"tipo_documento") == 'CARTAO_VISA'){ echo('selected') ; }?>><?php echo(getTText("cartao_visa",C_TOUPPER)); ?></option>
												<option value="CARTAO_MASTERCARD"   <?php if(getValue($objRS,"tipo_documento") == 'CARTAO_MASTERCARD'){ echo('selected') ; }?>><?php echo(getTText("cartao_mastercard",C_TOUPPER)); ?></option>
												<option value="CARTAO_AMEX"      	<?php if(getValue($objRS,"tipo_documento") == 'CARTAO_AMEX'){ echo('selected') ; }?>><?php echo(getTText("cartao_amex",C_TOUPPER)); ?></option>
											</select>
										</td>
									</tr>
								</table>		
						...
						<tr bgcolor="#FAFAFA"> 
							...
							<td>
								<table border="0px" cellpadding="0px" cellspacing="0px" width="100%">
									<tr>
										<td width="90px"><input name='var_dt_emissao' id='var_dt_emissao' class='edtext' value="<?php echo(dDate(CFG_LANG,getValue($objRS,"dt_emissao"),false)); ?>" type='text' maxlength='10' style='width:70px;' onKeyUp="Javascript:FormataInputData(this);" onKeyPress="Javascript:return validateNumKey(event);"  ><span class="texto_corpo_peq"></span></td>
										<td width="120px" align="right">*<b><?php echo(getTText("dt_vcto",C_UCWORDS)); ?></b>:&nbsp;</td>
										<td align="left"><input name='var_dt_vcto' id='var_dt_vcto' class='edtext' value="<?php echo(dDate(CFG_LANG,getValue($objRS,"dt_vcto"),false)); ?>" type='text' maxlength='10' style='width:70px;' onKeyUp="Javascript:FormataInputData(this);" onKeyPress="Javascript:return validateNumKey(event);"  ><span class="texto_corpo_peq"></span></td>
									</tr>
								</table>		
							</td>					
						</tr>
						<tr bgcolor="#FFFFFF">
							...
							<td>
								<select name="var_situacao" style="width:180px;" disabled="disabled">
						...
						<tr bgcolor="#FAFAFA"> 
							...
							<td valign="middle"> 
								<select name="var_cod_cfg_boleto" class="edtext" style="width:160px;">
									<?php echo(montaCombo($objConn," SELECT cod_cfg_boleto, descricao FROM cfg_boleto WHERE dtt_inativo IS NULL ORDER BY descricao ","cod_cfg_boleto","descricao",getValue($objRS, "cod_cfg_boleto"),"")); ?>
								</select>
							</td>
						</tr> 
						<tr bgcolor="#FFFFFF">

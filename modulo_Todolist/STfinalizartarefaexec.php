<?php
	...
	$dtRealizado	= request("var_todo_dt_realizado");
	$strLocation	= request("DEFAULT_LOCATION");
	$strJSAction	= request("JSCRIPT_ACTION");
	$boolMail		= FALSE;
	...
	try{
		...
		// SETA A TAREFA PARA REALIZADO
		$strSQL = "UPDATE tl_todolist SET situacao = 'fechado', dt_realizado = '".cDate(CFG_LANG,$dtRealizado,false)."' WHERE cod_todolist = ".$intCodDado;
		$objConn->query($strSQL);
...
?>
<?php if($strJSAction != ""){?>
	<script type="text/javascript"><?php echo($strJSAction);?></script>
<?php } else{?>
	<script type="text/javascript">document.location.href="<?php echo($strLocation);?>"</script>
<?php }?>
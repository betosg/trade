<?php
	...
	$strJSAction	 = request("var_js_action");
	
	$flagBG			 = request("var_flag_bg");
	...
	if(getsession(CFG_SYSTEM_NAME."_id_usuario") != getValue($objRS,"id_responsavel")){
		mensagem("err_sql_titulo","aviso_tarefa_nao_te_pertence","",$strLocation,"aviso",1);
		die();
	}
	...
	// TRATAMENTO COM OS CAMPOS DATA INICIAL, HORA INICIAL E PREVISÃO DE HORAS
	if(strpos(getValue($objRS,"prev_hr_ini"),":")){
		$arrHrIni = explode(":",getValue($objRS,"prev_hr_ini"));
		$strPrevHrIni1 = $arrHrIni[0];
		$strPrevHrIni2 = $arrHrIni[1];
	} else{
		$strPrevHrIni1 = "";
		$strPrevHrIni2 = "";
	}
	
	if(strpos(getValue($objRS,"prev_horas"),":")){
		$arrHoras = explode(":",getValue($objRS,"prev_horas"));
		$strPrevHoras1 = $arrHoras[0];
		$strPrevHoras2 = $arrHoras[1];
	} else{
		$strPrevHoras1 = "";
		$strPrevHoras2 = "";
	}
		...
		<script type="text/javascript">
			...
			function ok() {
				if(validaCampos()){
					...
					<?php }?>
					<?php if($strJSAction != ""){?>
						strJSAction = <?php echo($strJSAction);?>
					<?php }?>
					submeterForm();
				} else{
					return(false);
			...
			function aplicar() {
				if(validaCampos()){
					strLocation = "../modulo_Todolist/STupdtarefa.php?var_chavereg=<?php echo($intCodDado);?>&var_cod_atividade=<?php echo($intCodAtividade);?>&var_js_action=<?php echo($strJSAction);?>&var_location=<?php echo($strLocation);?>";
					submeterForm();
				} else{
					return(false);
				}
			...
			function submeterForm() {
				...
				document.formstatic.JSCRIPT_ACTION.value   = strJSAction;
				document.formstatic.submit();
			}
		</script>
	</head>
<body style="margin:10px;<?php echo((($intCodAtividade != "") || ($flagBG != "")) ? "background-color:#FFF;" : "background-image:url(../img/bgFrame_".CFG_SYSTEM_THEME."_main.jpg);");?>">
<!-- USO -->
<center>
<?php athBeginFloatingBox("710","",getTText("todolist",C_UCWORDS)." - (".getTText("alteracao",C_NONE).")",CL_CORBAR_GLASS_1); ?>
<form name="formstatic" action="STupdtarefaexec.php" method="post">
	...
	<input type="hidden" name="var_todo_cod_todolist" id="var_todo_cod_todolist" value="<?php echo($intCodDado);?>" />
		...
		<td align="left" valign="top" style="padding:10px 70px 10px 70px;">
			<table cellspacing="2" cellpadding="4" border="0" width="100%">
				...
				<tr bgcolor="<?php echo(getLineColor($strColor));?>">
					...
					<td align="left"><input type="text" name="var_todo_titulo" id="var_todo_titulo" value="<?php echo(getValue($objRS,"titulo"));?>" maxlength="250" size="60" /></td>
				</tr>
				<tr bgcolor="<?php echo(getLineColor($strColor));?>">
				...
				<tr bgcolor="<?php echo(getLineColor($strColor));?>">
					...
					<td align="left">
						<select name="var_todo_categoria" id="var_todo_categoria" style="width:150px;">
							<?php echo(montaCombo($objConn,"SELECT tl_categoria.cod_categoria, UPPER(tl_categoria.cod_categoria||' - '||tl_categoria.nome) AS nome FROM tl_categoria WHERE dtt_inativo IS NULL","cod_categoria","nome",getValue($objRS,"categoria")));?>
						</select>
					</td>
				</tr>
				...
				<tr bgcolor="<?php echo(getLineColor($strColor));?>">
					...
					<td align="left">
						...
						<select name="var_grupo_usuario_executor" id="var_grupo_usuario_executor" style="width:80px;" onChange="limpaSelect('var_todo_id_ult_executor');ajaxDetailDataLoader((this.value != '') ? 'SELECT DISTINCT id_usuario, id_usuario FROM sys_usuario WHERE dtt_inativo IS NULL AND oculto = FALSE AND grp_user = \'' + this.value + '\' ORDER BY id_usuario' : '','ajaxMontaCombo','var_todo_id_ult_executor','','loader_ajax_executor',this.value);">
							<option value=""><?php echo(getTText("grupo_reticencias",C_NONE));?></option>
							<?php if($intCodAtividade != ""){?>
							<option value="equipe" selected="selected"><?php echo(getTText("equipe",C_TOUPPER));?></option>
							<?php } else{?>
							<?php echo(montaCombo($objConn,"SELECT DISTINCT grp_user FROM sys_usuario ORDER BY grp_user","grp_user","grp_user",getValue($objRS,"grupo_executor")));?>
							<?php }?>
						</select>
						...
						<select name="var_todo_id_ult_executor" id="var_todo_id_ult_executor" style="width:100px;">
							...
							<?php if($intCodAtividade != ""){?>
								<?php echo(montaCombo($objConn,"SELECT DISTINCT id_usuario FROM bs_equipe WHERE dtt_inativo IS NULL AND cod_atividade = ".$intCodAtividade." ORDER BY id_usuario","id_usuario","id_usuario",getValue($objRS,"id_ult_executor")));?>
							<?php } else{?>
				...
				<tr bgcolor="<?php echo(getLineColor($strColor));?>">
					...
					<td align="left">
						<input type="text" name="var_todo_prev_dt_ini" id="var_todo_prev_dt_ini" size="12" value="<?php echo(dDate(CFG_LANG,getValue($objRS,"prev_dt_ini"),false));?>" onKeyPress="return validateNumKey(event);" onKeyDown="FormataInputData(this,event);" />
						&nbsp;
						...
						<select name="var_todo_prev_hr_ini_1" id="var_todo_prev_hr_ini_1" style="width:45px;">
							...
							<option value="<?php echo((strlen($auxCounter) < 2) ? "0".$auxCounter : $auxCounter);?>" <?php echo((($strPrevHrIni1 == $auxCounter) || ($strPrevHrIni1 == "0".$auxCounter)) ? "selected='selected'" : "");?> ><?php echo((strlen($auxCounter) < 2) ? "0".$auxCounter." h" : $auxCounter." h");?></option>
							<?php }?>
						</select>
						&nbsp;
						<select name="var_todo_prev_hr_ini_2" id="var_todo_prev_hr_ini_2" style="width:55px;">
							...
							<option value="<?php echo((strlen($auxCounter) < 2) ? "0".$auxCounter : $auxCounter);?>" <?php echo((($strPrevHrIni2 == $auxCounter) || ($strPrevHrIni2 == "0".$auxCounter)) ? "selected='selected'" : "");?> ><?php echo((strlen($auxCounter) < 2) ? "0".$auxCounter." min" : $auxCounter." min");?></option>
							<?php }?>
							<?php }?>
						</select>
				...
				<tr bgcolor="<?php echo(getLineColor($strColor));?>">
					...
					<td align="left">
						<select name="var_todo_prev_horas_1" id="var_todo_prev_horas_1" style="width:45px;">
							<?php for($auxCounter = 0; $auxCounter <= 23; $auxCounter++){?>
							<option value="<?php echo((strlen($auxCounter) < 2) ? "0".$auxCounter : $auxCounter);?>" <?php echo((($strPrevHoras1 == $auxCounter) || ($strPrevHoras1 == "0".$auxCounter)) ? "selected='selected'" : "");?> ><?php echo((strlen($auxCounter) < 2) ? "0".$auxCounter." h" : $auxCounter." h");?></option>
							<?php }?>
						</select>
						&nbsp;
						<select name="var_todo_prev_horas_2" id="var_todo_prev_horas_2" style="width:55px;">
							<?php for($auxCounter = 0; $auxCounter <= 55; $auxCounter++){?>
							<?php if(($auxCounter % 5) == 0){?>
							<option value="<?php echo((strlen($auxCounter) < 2) ? "0".$auxCounter : $auxCounter);?>" <?php echo((($strPrevHoras2 == $auxCounter) || ($strPrevHoras2 == "0".$auxCounter)) ? "selected='selected'" : "");?> ><?php echo((strlen($auxCounter) < 2) ? "0".$auxCounter." min" : $auxCounter." min");?></option>
							<?php }?>
							<?php }?>
						</select>
					</td>
				...
				<tr bgcolor="<?php echo(getLineColor($strColor));?>">
					...
					<td align="left">
						<input type="text" name="var_todo_arquivo_anexo" id="var_todo_arquivo_anexo" value="<?php echo(getValue($objRS,"arquivo_anexo"));?>" size="50" readonly="true" title="<?php echo(getTText("anexo",C_NONE));?>">
				...
				<tr bgcolor="<?php echo(getLineColor($strColor));?>">
					...
					<td align="left"><textarea name="var_todo_descricao" id="var_todo_descricao" style="width:350px;height:120px;"><?php echo(getValue($objRS,"descricao"));?></textarea></td>
				</tr>
				<tr>

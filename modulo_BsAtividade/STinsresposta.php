		  ...
		  <td align="left" valign="top" style="padding:10px 75px 10px 75px;">
				<table cellspacing="2" cellpadding="4" border="0" width="100%">
					...
					<tr bgcolor="<?php echo(getLineColor($strColor));?>">
						...
						<td align="left">
							<select name="var_grupo_usuario_to" id="var_grupo_usuario_to" style="width:80px;" onChange="limpaSelect('var_todo_id_to');ajaxDetailDataLoader((this.value != '') ? 'SELECT DISTINCT id_usuario, id_usuario FROM sys_usuario WHERE dtt_inativo IS NULL AND oculto = FALSE AND grp_user = \'' + this.value + '\' ORDER BY id_usuario' : '','ajaxMontaCombo','var_todo_id_to','','loader_ajax_responsavel');">
								<?php echo((getsession(CFG_SYSTEM_NAME."_id_usuario") == getValue($objRS,"id_responsavel")) ? montaCombo($objConn,"SELECT DISTINCT grp_user FROM sys_usuario ORDER BY grp_user","grp_user","grp_user",getValue($objRS,"grupo_executor")) : montaCombo($objConn,"SELECT DISTINCT grp_user FROM sys_usuario ORDER BY grp_user","grp_user","grp_user",getValue($objRS,"grupo_responsavel")));?>
							</select>
							&nbsp;
							<select name="var_todo_id_to" id="var_todo_id_to" style="width:100px;">
								<?php echo((getsession(CFG_SYSTEM_NAME."_id_usuario") == getValue($objRS,"id_responsavel")) ? montaCombo($objConn,"SELECT DISTINCT id_usuario FROM sys_usuario WHERE dtt_inativo IS NULL AND oculto = FALSE AND grp_user = '".getValue($objRS,"grupo_executor")."' ORDER BY id_usuario","id_usuario","id_usuario",getValue($objRS,"id_ult_executor")) : montaCombo($objConn,"SELECT DISTINCT id_usuario FROM sys_usuario WHERE dtt_inativo IS NULL AND oculto = FALSE AND grp_user = '".getValue($objRS,"grupo_responsavel")."' ORDER BY id_usuario","id_usuario","id_usuario",getValue($objRS,"id_responsavel")));?>
							</select>

...
foreach($strAux as $strCampo => $strValor){ 
	...
	switch(strtolower($strAuxType)){
			...
			case "dateinterval": 
							 ...
							 $strRegExData = "(" . $strREDt1 . "|" . $strREDt2 . ")";
							 ...
							 if(preg_match("/" . $strRegExData . "( *)(E|AND|Y|ATÉ|UNTIL|HASTA|-)+( *)" . $strRegExData . "/i",$strAuxValue) != 0){
								...
								$dateTmpDate1 = cDate(CFG_LANG,trim($arrTmpDates[0]),false);
								$dateTmpDate2 = cDate(CFG_LANG,trim($arrTmpDates[1]),false);
								
								$strAuxValue  = " BETWEEN '" . $dateTmpDate1 . "' AND '" . $dateTmpDate2 . "'";
							 }

				...
				<?php
					...
					while($boolContinuaRS){
						if($strDialogGrp != getValue($objRS,"dlg_grp")) {
							...
							if(getValue($objRS,"dlg_grp") == "000"){ 
								...
								$strHeader = "
											  <table border=\"0\" cellpadding=\"0\" cellspacing=\"0\" width=\"100%\">
												<tr>
													<td align=\"left\" width=\"99%\" class=\"padrao_gde\">" . getTText(getValue($objRS,"nome_tabela"),C_TOUPPER) . " (" . $strTituloHeader . ")" . "</td>
							...
							athBeginFloatingBox("725","none",$strHeader,CL_CORBAR_GLASS_1);	?>
      <table id="dialog_cont_<?php echo(getValue($objRS,"dlg_grp")); ?>" width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF" style="border:1px solid #A6A6A6;<?php echo($strCollapse); ?>">
	   <form name="formeditor_<?php echo(getValue($objRS,"dlg_grp")); ?>" action="<?php echo($arrCfgDialog["dlg_action"]); ?>" method="post">
			...
			<table width="550" border="0" cellspacing="0" cellpadding="4">
				<?php
							if($strOperacao != "INS" && getValue($objRS,"dlg_grp") == "000"){
								echo("
										<tr>
											<td width=\"1%\" align=\"right\"><label><strong>*" . getTText($arrCfgDialog["dlg_campo_chave"],C_NONE) . ":</strong></label>&nbsp;</td>
											<td>" . $intCodDado . "</td>
										</tr>
									");
							}
						...
						if(!$boolContinua && ($strDialogGrp == "000" || ($strOperacao != "VIE" && $strOperacao != "DEL"))){
							...
							echo("  <td width=\"1%\" align=\"right\" valign=\"top\" nowrap style=\"padding-right:5px;\">
									...
									<td nowrap" . ((getValue($objRS,"nowrap")) ? "" : " align=\"left\" width=\"99%\" ") . ">
								");
							...
							if(getValue($objRS,"nowrap") && !$boolMesmaLinha) { 
								...
								if($boolMesmaLinha) { $boolMesmaLinha = true; } else { $boolMesmaLinha = false; }
							}
							...
							switch($strClasse){								
								...
								/******************* FILEs *******************/
								case "FILE":
									echo("<input type=\"text\" name=\"" . $strComponente . "\" id=\"" . $strComponente . "_" . getValue($objRS,"dlg_grp") . "\" value=\"" . $strValor . "\" size=\"50\" readonly=\"true\" title=\"" . getTText($strNomeField,C_NONE) . "\">");
									echo("<input type=\"button\" name=\"btn_uploader\" value=\"Upload\" class=\"inputclean\" onClick=\"callUploader('formeditor_" . getValue($objRS,"dlg_grp") . "','" . $strComponente ."','\\\\" . str_replace("\\","\\\\",replaceParametersSession(getValue($objRS,"file_dir_arquivos"))) . "\\\\','" . getValue($objRS,"file_prefix") . "','" . getValue($objRS,"file_flag_sufix") . "');\">");
								break;//*/
								...
								/******************* MEMOs ********************/
								case "MEMO":
									echo("<textarea name=\"" . $strComponente . "\" id=\"" . $strComponente . "_" . getValue($objRS,"dlg_grp") . "\" cols=\"" . getValue($objRS,"param_memo_cols") . "\" rows=\"" . getValue($objRS,"param_memo_rows") . "\"");
									echo(" " . $strChamadas);
									//echo(" title=\"" . getTText($strNomeField,C_NONE) . "\" tabindex=\"" . $intI . "\">" . $strValor . "</textarea>");
								...
								case "LABEL":
									if($strValor != "") {
										...
										if(getValue($objRS,"classe") == "FILE"){
											...
											if(ereg("\.(jp[e]?g|gif|png)$",$strValor)){												
								...
								case "RADIO": 
									...
									foreach($arrValues as $strArrValues){
										...
										//echo(" class=\"inputclean\" tabindex=\"" . $intI . "\">" . getTText(trim($arrCaptions[$intI]),C_NONE));
										echo(" " . $strChamadas);
						...
						if($strDialogGrp != getValue($objRS,"dlg_grp")) {
							...
							if($strDialogGrp != "000"){
								...
								echo("<tr><td colspan=\"2\" style=\"padding-left:10px;\"><iframe src=\"" . $strIframeDialogSrc . "\" name=\"grid_dialog_" . $strDialogGrp . "\" id=\"grid_dialog_" . $strDialogGrp . "\" width=\"100%\" height=\"" . $intIframeDialogHeight . "\" frameborder=\"0\" scrolling=\"auto\"></iframe></td></tr>");
							}
				?>
			</table>
		  </td>

...
try{
	if (strpos(CFG_DB,"abfm")>0){
    //Busca pelo ID ou CPF  -0 conforme o que foi digitado (um OU outro serve)
		$strSQL  = " SELECT DISTINCT sys_usuario.cod_usuario, sys_usuario.id_usuario, case when cad_pf.email is null then sys_usuario.email  else cad_pf.email end as email";
		...
		$strSQL  = " SELECT DISTINCT sys_usuario.cod_usuario, sys_usuario.id_usuario, sys_usuario.email ";
...
if ($objResult->rowCount() > 0){
		foreach($objResult as $objRS) {
			try {
				...
				$strCOD   = getValue($objRS, "cod_usuario");
				...
				if(trim($strEMAIL) != "") {
					...
					$strCorpo = getTText("esqueceu_senha_nome_email", C_NONE) . "<b>" . $strID . "</b><br>" . getTText("esqueceu_senha_novasenha_email",C_NONE) . "<b>" . $strPASS . "</b><br><br>";
				...
				<?php athBeginFloatingBox("415","",getTText("esqueceu_senha_tit",C_NONE),CL_CORBAR_GLASS_1); ?>
					<table border="0" bgcolor="#FFFFFF" align="center" cellspacing="0" cellpadding="0" width="390" style="border:1px #A6A6A6 solid;">
						...
						<tr>
							...
							<td align="right" width="210" ><img src="<?php echo(CFG_SYSTEM_LOGO_CAPA); ?>" border="0">&nbsp;&nbsp;</td>
						</tr>

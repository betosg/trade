...
<html>
	<head>
		...
		<!--
			...
			function ok() {
				strLocation = "../modulo_FinContaPagarReceberFornec/STifrlancamentoSinog.php?var_chavereg=<?php echo($intCodTitulo)?>";
				submeterForm();
			}

			function cancelar() {
				document.location.href = "../modulo_FinContaPagarReceberFornec/STifrlancamentoSinog.php?var_chavereg=<?php echo($intCodTitulo)?>";
			}

			function aplicar() {
				strLocation = "../modulo_FinContaPagarReceberFornec/STupdlctoordinarioSinog.php?var_chavereg=<?php echo($intCodTitulo)?>";
				submeterForm();
			}
		...
		<?php athBeginFloatingBox("600","none",$strTITULO . " - " . getTText("lcto_ord",C_UCWORDS),CL_CORBAR_GLASS_1); ?>
			<table border="0" width="100%" bgcolor="#FFFFFF" style="border:1px #A6A6A6 solid;">
			  <form name="formconf" action="STinslctoordinarioexec.php" method="post">
				...
				<input type="hidden" name="var_tipo_conta" 	
				 value="<?php echo((getValue($objRS,"pagar_receber") != false) ? "pagar" : "receber"); ?>">
										...
										<tr valign="middle">
											<td style="padding-right:3px;">
												<select name="var_cod_job" class="edtext" style="width:230px;">
													<?php echo(montaCombo($objConn," SELECT cod_job, nome FROM fin_job WHERE dtt_inativo IS NULL ORDER BY ordem, nome ","cod_job","nome",getValue($objRS,'cod_job'))); ?>
												</select>	
											</td>
										</tr>
									</table>

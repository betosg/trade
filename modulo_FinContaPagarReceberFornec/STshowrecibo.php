<?php
	...
	// VERIFICA SE TITULO É AGRUPADO OU NÃO, PARA REDIRECT CORRETO
	try{
		$strSQL    = "SELECT tipo FROM fin_conta_pagar_receber WHERE cod_conta_pagar_receber = ".$intCodDado.";";
		//die($strSQL);
		$objResultTipo = $objConn->query($strSQL);
		$objRSTP 	   = $objResultTipo->fetch();
	}catch(PDOException $e) {
		mensagem("err_sql_titulo","err_sql_desc",$e->getMessage(),"","erro",1);
		die();
	}
	...
	try{
		switch(strtolower(getValue($objRSTP,"tipo"))){
			case "cad_pf" :
			$strSQL = "
				SELECT 
				  	  cad_pf.nome AS nome
			...
			$strSQL = "
				SELECT 
				  	  cad_pj_fornec.razao_social AS nome
					, cad_pj_fornec.cnpj AS doc
					, 'CNPJ' AS tipo_doc
				FROM  cad_pj_fornec
				INNER JOIN fin_conta_pagar_receber ON (fin_conta_pagar_receber.codigo = cad_pj_fornec.cod_pj_fornec)
				WHERE fin_conta_pagar_receber.cod_conta_pagar_receber = ".$intCodDado;
			break;
		}	
		//die($strSQL);
	...
	$strVALUE  = "";
	$strVALUE .= getValue($objRSJ,"nome");
	$strVALUE .= (getValue($objRSJ,"doc") != "") ? " (".getValue($objRSJ,"tipo_doc")." ".getValue($objRSJ,"doc").")" : "";
	...
	<head>
		...
		<script type="text/javascript">
			...
			function ok() {
				...
				document.location.href = "../modulo_FinContaPagarReceberFornec/STifrrecibos.php?var_chavereg=<?php echo($intCodDado);?>";
			}

<?php
	...
	// REQUEST - DADOS PESSOA JURÍDICA
	$flagTipoPJ  = request("var_tipo_pj");
	$strCNPJ     = request("var_pj_cnpj");
	$strRAZAO    = request("var_pj_razao");
	$intCNAE     = request("var_pj_cnae_grupo");
	$strEnderCEP = request("var_pj_ender_cep");
	$strEnderLOG = request("var_pj_ender_logradouro");
	$strEnderNUM = request("var_pj_ender_numero");
	$strEnderCOM = request("var_pj_ender_complemento");
	$strEnderBAI = request("var_pj_ender_bairro");
	$strEnderCID = request("var_pj_ender_cidade");
	$strEnderEST = request("var_pj_ender_estado");
	$strEnderPAI = request("var_pj_ender_pais");
	$intCodPJ 	 = request("var_pj_codigo");
	...
	try{
		...
		if($flagTipoPJ == "NEW_PJ"){
			// NEW_PJ = Insere nova PJ
			$strSQL = " INSERT INTO cad_pj (categoria, cod_cnae_n3, cnpj, razao_social, nome_fantasia, endprin_cep, endprin_logradouro, endprin_numero, endprin_complemento, endprin_bairro, endprin_cidade, endprin_estado, endprin_pais, sys_usr_ins, sys_dtt_ins) 
						VALUES ('CONTRIBUINTE',".$intCNAE.",'".prepStr($strCNPJ)."','".prepStr($strRAZAO)."','".prepStr($strRAZAO)."','".prepStr($strEnderCEP)."','".prepStr($strEnderLOG)."','".prepStr($strEnderNUM)."','".prepStr($strEnderCOM)."','".prepStr($strEnderBAI)."','".prepStr($strEnderCID)."','".prepStr($strEnderEST)."','".prepStr($strEnderPAI)."','".prepStr(getsession(CFG_SYSTEM_NAME."_id_usuario"))."',CURRENT_TIMESTAMP);";
		...
		if($flagTipoPF == "NEW_PF"){
			...
			$strSQL = "	INSERT INTO cad_pf (cpf, rg, nome, sexo, obs, sys_usr_ins, sys_dtt_ins) 
						VALUES ('".prepStr($strCPF)."','".prepStr($strRG)."','".prepStr($strNOME)."','".prepStr($chrSEXO)."','".prepStr($strPFOBS)."','".prepStr(getsession(CFG_SYSTEM_NAME."_id_usuario"))."',CURRENT_TIMESTAMP);";
			$objConn->query($strSQL);
			...
			$strSQL = "	INSERT INTO relac_pj_pf (cod_pj, cod_pf, tipo, funcao, departamento, dt_admissao, obs, categoria, sys_usr_ins, sys_dtt_ins) 
						VALUES (".$intCodPJ.",".$intCodPF.",'".prepStr($strTIPO)."','".prepStr($strFUNCAO)."','".prepStr($strDEPART)."','".cDate(CFG_LANG,$dttADMISSAO,false)."','".prepStr($strVAGAOBS)."','".prepStr($strCATEG)."','".prepStr(getsession(CFG_SYSTEM_NAME."_id_usuario"))."',CURRENT_TIMESTAMP);";
			$objConn->query($strSQL);
		...
		if($flagTipoPF == "PJ_PF"){
			...
			$objRS	   = $objResult->fetch();
			$intCodRE  = getValue($objRS,"cod_pj_pf");
		}
		...
		if($flagTITULO == "TIT_NEW"){
			...
			$strSQL = " INSERT INTO fin_lcto_ordinario (cod_conta_pagar_receber, tipo, codigo, cod_conta, cod_plano_conta, cod_centro_custo, historico, num_lcto, dt_lcto, vlr_multa, vlr_juros, vlr_desc, vlr_lcto, tipo_documento, num_documento, sys_usr_ins, sys_dtt_ins) 
						VALUES (".$intCodTIT.",'cad_pj',".$intCodPJ.",".$intCCONTA.",".$intCPLANO.",".$intCCUSTO.",'".$strHIST."','".$strNUMLCTO."',CURRENT_DATE,0,0,0,".$dblVALOR.",'".$strTPDOC."','".$strNUMDOC."','".prepStr(getsession(CFG_SYSTEM_NAME."_id_usuario"))."',CURRENT_TIMESTAMP);";
			$objConn->query($strSQL);
		}
		...
		$strSQL = "	UPDATE sd_homologacao 
					SET dtt_homologacao = '".cDate(CFG_LANG,$dttHOMOLOG,true)."'
					  , obs = '".prepStr($strHOMOOBS)."'
		...
		$strSQL = " UPDATE relac_pj_pf 
					SET dt_demissao = '".cDate(CFG_LANG,$dttHOMOLOG,true)."'
					  , sys_dtt_upd = CURRENT_TIMESTAMP

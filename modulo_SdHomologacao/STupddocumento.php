<?php
	...
	try{
		$strSQL = "
			SELECT 
				...
				, sd_homologacao_documento.cod_homologacao
			...
			WHERE sd_homologacao_documento.cod_homologacao_documento = ".$intCodDado;
		//die($strSQL);
	...
	try{
		$strSQL = "
			SELECT sd_homologacao.situacao FROM sd_homologacao WHERE sd_homologacao.cod_homologacao = ".getValue($objRS,"cod_homologacao");
	...
	if(getValue($objRSV,"situacao") != "aberto"){
		mensagem("err_sql_titulo","err_sql_desc_card","Esta HOMOLOGAÇÃO não está ABERTA, para EDIÇÃO DE DOCUMENTOS.","STdocumentos.php?var_chavereg=".getValue($objRS,"cod_homologacao"),"aviso",1,"");
		die();	
	}
		...
		<script type="text/javascript">
			...
			function ok() {
				...
				if(strMSG != ""){
					...
					strLocation = "../modulo_SdHomologacao/STdocumentos.php?var_chavereg=<?php echo(getValue($objRS,"cod_homologacao"));?>";
					submeterForm();
				}
			}

			function cancelar() {
				document.location.href = "../modulo_SdHomologacao/STdocumentos.php?var_chavereg=<?php echo(getValue($objRS,"cod_homologacao"));?>";
			}
			...
			function aplicar() {
				...
				if(strMSG != ""){
					...
					strLocation = "../modulo_SdHomologacao/STupddocumento.php?var_chavereg=<?php echo($intCodDado);?>";
					submeterForm();
				}
			}
...
<table cellpadding="0" cellspacing="0" border="0" height="100%" width="660" bgcolor="#FFFFFF" class="table_master">
	...
	<tr>
		<td align="left" valign="top" style="padding:10px 80px 10px 80px;">
			...
			<form name="formstatic" id="formstatic" action="STupddocumentoexec.php" method="post">
				<input type="hidden" name="var_cod_homologacao" value="<?php echo(getValue($objRS,"cod_homologacao"));?>" />
				...
				<tr bgcolor="<?php echo(getLineColor($strColor));?>">
					...
					<td width="77%" align="left">
						<select name="var_tipo" id="var_tipo" style="width:100px;">
							<option value="termo" <?php echo((getValue($objRS,"tipo") == "TERMO") ? "selected=selected" : "");?>>TERMO</option>
							<option value="ressalva" <?php echo((getValue($objRS,"tipo") == "RESSALVA") ? "selected=selected" : "");?>>RESSALVA</option>
							<option value="declaracao" <?php echo((getValue($objRS,"tipo") == "DECLARACAO") ? "selected=selected" : "");?>>DECLARAÇÃO</option>
						</select>
					</td>
				</tr>
				<tr bgcolor="<?php echo(getLineColor($strColor));?>">
					<td width="23%" align="right" valign="top"><strong>*<?php echo(getTText("titulo",C_UCWORDS));?>:</strong></td>
					<td width="77%" align="left"><input type="text" name="var_titulo" id="var_titulo" value="<?php echo(getValue($objRS,"titulo"));?>" size="65" maxlength="255" /></td>
				</tr>
				<tr bgcolor="<?php echo(getLineColor($strColor));?>">
					...
					<td width="77%" align="left">
						...
						<tr>
							<td width="75%"><textarea name="var_html_texto" id="var_html_texto" rows="10" cols="80"><?php echo(getValue($objRS,"html_texto"));?></textarea>&nbsp;<br /><span class="comment_med">Edite o HTML do texto clicando no link ao lado</span></td>
							<td width="25%" valign="middle" align="left"><img src="../img/bt_editHTML.gif" border="0" style="cursor:pointer" onClick="EditaCampos('cod_homologacao_documento','<?php echo(getValue($objRS,"cod_homologacao_documento"));?>','sd_homologacao_documento','html_texto',document.getElementById('var_html_texto').innerHTML,'../modulo_SdHomologacao/STupddocumento.php?var_chavereg=<?php echo(getValue($objRS,"cod_homologacao_documento"));?>','<?php echo(getValue($objRS,"cod_homologacao"));?>');" /></td>
						</tr>
						</table>
					</td>
				</tr>
				<tr bgcolor="<?php echo(getLineColor($strColor));?>">
					<td width="23%" align="right" valign="top"><strong><?php echo(getTText("inativo_em",C_UCWORDS));?>:</strong></td>
					<td width="77%" align="left"><input type="text" name="var_dtt_inativo" value="<?php echo(dDate(CFG_LANG,getValue($objRS,"dtt_inativo"),false));?>" size="12" maxlength="10" onKeyPress="return validateNumKey(event);" onKeyDown="FormataInputData(this,event);" /></td>
				</tr>
				<tr>
...
<script type="text/javascript">
  ...
  resizeIframeParent('<?php echo(CFG_SYSTEM_NAME); ?>_detailiframe_<?php echo(getValue($objRS,"cod_homologacao")); ?>',20);
  // ----------------------------------------------------------------------------------------------------------
</script>
</html>
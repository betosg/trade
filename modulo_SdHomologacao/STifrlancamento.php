...
<?php
	...
	// REQUESTS
	$intCodTitulo  			 = request("var_cod_titulo");  	// cod_conta_pagar_receber
	 	...
	 	<script language="javascript">
			...
			function removeLancamento(prCodLancamento){
				if(confirm("Tem certeza que deseja remover este lançamento?")){
					window.location = 'STremovelancamento.php?var_cod_titulo=<?php echo($intCodTitulo);?>&var_chavereg=<?php echo(request("var_chavereg"));?>&var_cod_lcto_ordinario=' + prCodLancamento;
				}
			}
		</script>
	</head>
			...
			<td align="left">
				<?php
					...
					athBeginCssMenu();
						...
						athBeginCssSubMenu();
							athCssMenuAddItem("STinslctoordinario.php?var_cod_titulo=".$intCodTitulo."&var_chavereg=".request("var_chavereg"),"_self",getTText("lcto_inserir",C_NONE));
						athEndCssSubMenu();
					athEndCssMenu();		
				?>
			</td>
		...
		?>
			<tr bgcolor="<?php echo(getLineColor($strColor)); ?>">	
				...
				<td style="vertical-align:middle;" align="center">
					...
					onClick="AbreJanelaPAGE('../modulo_FinContaPagarReceber/STshowrecibolcto.php?var_chavereg=<?php echo(getValue($objRS,"cod_lcto_ordinario"));?>','800','800');" />
				</td>

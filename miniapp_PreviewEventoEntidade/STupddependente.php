...
<?php
	...
	try{
		...
		$strSQL = $strSQL = "           SELECT nome ";
		$strSQL = $strSQL . "      , TO_CHAR(cad_funcionario_dependente.data_nasc, 'DD/MM/YYYY') AS dt_nasc ";
		$strSQL = $strSQL . "      , parentesco ";
		$strSQL = $strSQL . "      , cpf ";
		$strSQL = $strSQL . "      , cod_dependente ";
		$strSQL = $strSQL . "      , cad_funcionario_dependente.cod_parentesco ";
...
//		$intCodInd = getValue($objRSCF,"cod_cadastro");
...
<html>
	<head>
		...
		<script language="javascript" type="text/javascript">
			...
			function aplicar() {
				document.location.href = "../miniapp_cadfuncionario_dependente/STupdcontato.php?var_cf_sub=<?php echo($intCodDado);?>&var_cod_cad=<?php echo $var_cod_cad;?>";			
				submeterForm();
			}
...
<table cellpadding="0" cellspacing="0" border="0" width="100%">
	<tr>
	<td align="center">
	<?php athBeginFloatingBox("520","none",getTText("dependente_inserir_title",C_NONE)." (". getTText("atualizar",C_NONE).")",CL_CORBAR_GLASS_1); ?>
				...
				<table cellspacing="2" cellpadding="3" border="0" width="100%">
					...
					<tr bgcolor="#FFFFFF">
						...
						<td width="73%" align="left" class="destaque_gde">
							<strong><?php echo(getTText("dados_contato_ins",C_TOUPPER));?></strong>
						</td>
					</tr>
					...
					<!-- CAMPOS: OS CAMPOS SÃO DIVIDOS BASICAMENTE EM UM ROTULO + CAMPO / VALOR POR LINHA -->
                            ...
                            <td width="27%" align="right"><strong><?php echo(getTText("cod_dependente",C_UCWORDS));?>:</strong></td>
                            <td width="73%" align="left"><?php echo(getValue($objRS,"cod_dependente"))?></td>
                        </tr>
                        <tr bgcolor="<?php echo(getLineColor($strColor));?>">
                            <td width="27%" align="right"><strong><?php echo(getTText("cpf",C_UCWORDS));?></strong></td>
                            <td width="73%" align="left"><input type="text" name="DBVAR_STR_CPF" id="DBVAR_STR_CPF" maxlength="11" size="15" value='<?php echo(getValue($objRS,"cpf"))?>'  onblur='validaCPF();' onkeypress='validateNumKey(this);'></span></td>
                        </tr>
                        <tr bgcolor="<?php echo(getLineColor($strColor));?>">
                            <td width="27%" align="right"><strong><?php echo(getTText("nome_dependente",C_UCWORDS));?>:</strong></td>
                            <td width="73%" align="left"><input name="DBVAR_STR_NOME" value='<?php echo(getValue($objRS,"nome"))?>' size="50" maxlength="100" ></td>                            
                        </tr>						
                        <tr bgcolor="<?php echo(getLineColor($strColor));?>">
                            <td width="27%" align="right"><strong><?php echo(getTText("datanascmt",C_UCWORDS));?></strong></td>
                            <td width="73%" align="left"><input type="text" value='<?php echo(getValue($objRS,"dt_nasc"))?>' name="DBVAR_STR_DATA_NASC" maxlength="10" size="15" onkeyup='FormataInputData(this);'onkeypress='validateNumKey(this);'></span></td>
                        </tr>                        
                        <tr bgcolor="<?php echo(getLineColor($strColor));?>">
                            ...
                            <td width="73%" align="left">
								<select name="DBVAR_STR_COD_PARENTESCO" id="DBVAR_STR_COD_PARENTESCO" style="width:160px;" onChange="ajaxShowCombo(this.value);">
                                    <option value='<?php echo(getValue($objRS,"cod_parentesco"))?>' selected><?php echo(getValue($objRS,"parentesco"))?></option>
